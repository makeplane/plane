...
installation_summary() {
    ...
    echo "2. Start Plane Network service first (required for container DNS resolution):"
    echo "   systemctl start --user plane-nw-network.service"
    echo "   Note: For container hostname resolution, ensure aardvark-dns is installed"
    echo "   (e.g. apt install aardvark-dns or dnf install podman-plugins)."
    echo "3. Start all Plane services"
    echo "   systemctl start --user plane-{db,redis,mq,minio}.service"
    echo "   systemctl start --user {api,worker,beat-worker,migrator,monitor,automation-consumer,outbox-poller}.service"
    echo "   systemctl start --user {pi-db-init,pi-migrator,pi-api,pi-beat,pi-worker}.service"
    echo "   systemctl start --user {web,space,admin,live,iframely,proxy}.service"
    echo ""
    ...
    echo "   systemctl status --user {api,worker,beat-worker,migrator,monitor,automation-consumer,outbox-poller}.service"
    echo "   systemctl status --user {pi-db-init,pi-migrator,pi-api,pi-beat,pi-worker}.service"
    echo "   systemctl status --user {web,space,admin,live,iframely,proxy}.service"
    echo ""
