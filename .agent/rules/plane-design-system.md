  ...
  <Dialog.Panel width={EDialogWidth.MD}>
    <div className="p-6">
      ...
      <div className="mt-4 space-y-4">
        {/* form content — use text-13, bg-layer-2 for inputs */}
      </div>
      <div className="mt-6 flex justify-end gap-2">
        <Button variant="secondary" onClick={handleClose}>Cancel</Button>
        <Button variant="primary" onClick={() => void handleSubmit()}>Create</Button>
      </div>
    </div>
  </Dialog.Panel>
</Dialog>
        ...
        >
          <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-surface-1 text-left shadow-raised-200 transition-all sm:my-8 sm:w-[40rem]">
            ...
            <div className="flex justify-end gap-2 p-4 sm:px-6">
              <Button variant="secondary" onClick={handleClose}>Cancel</Button>
              <Button variant="primary" onClick={() => void handleSubmit()}>Confirm</Button>
            </div>
          </Dialog.Panel>
        </Transition.Child>
      </div>
...
function MyPage() {
  return (
    <div>
      <div className="flex justify-between border-b px-4 py-3">  {/* custom header */}
        <h1>Title</h1>
...
<Breadcrumbs>
  <Breadcrumbs.BreadcrumbItem type="text" link={<BreadcrumbLink label={t("dashboards")} href={`/${slug}/dashboards`} />} />
