...
export const ScheduleCard = observer(function ScheduleCard({ schedule }: Props) {
  ...
  return (
    <Card
      ...
      className="cursor-pointer transition-colors hover:border-accent-strong"
      onClick={() => void navigate(`/calendar/${schedule.id}`)}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 space-y-1">
          <div className="flex items-center gap-2">
            <span className="truncate text-body-sm-semibold text-primary">{schedule.name}</span>
            {schedule.is_default && (
              <span className="inline-flex shrink-0 items-center gap-1 rounded bg-accent-subtle px-1.5 py-0.5 text-caption-sm-medium text-accent-primary">
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {WEEKDAY_BADGE_LABELS.map((label, index) => {
          ...
          return (
            <span
              key={index}
              className={`rounded border px-2 py-0.5 text-caption-sm-medium ${
                active
