...
import {
  ...
  type TInstanceGithubAuthenticationConfigurationKeys,
} from "@plane/types";
...
import {
  ControllerInput,
  type TControllerInputFormField,
} from "@/components/common/controller-input";
  ...
  const onSubmit = async (formData: GithubConfigFormValues) => {
    ...
    await updateInstanceConfigurations(payload)
      .then((response = []) => {
        ...
        reset({
          GITHUB_CLIENT_ID: response.find(
            (item) => item.key === "GITHUB_CLIENT_ID"
          )?.value,
          GITHUB_CLIENT_SECRET: response.find(
            (item) => item.key === "GITHUB_CLIENT_SECRET"
          )?.value,
          GITHUB_ORGANIZATION_ID: response.find(
            (item) => item.key === "GITHUB_ORGANIZATION_ID"
          )?.value,
        });
      })
