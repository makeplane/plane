...
// plane imports
import { EIssueFilterType, EUserPermissionsLevel, GLOBAL_VIEW_TRACKER_ELEMENTS } from "@plane/constants";
...
export const TeamspaceProjectWorkItemAppliedFiltersRoot: React.FC = observer(() => {
  // router
  ...
  // derived values
  const viewDetails = teamspaceId && projectId ? getViewById(teamspaceId.toString(), projectId.toString()) : null;
  ...
  const handleUpdateView = () => {
    if (!workspaceSlug || !teamspaceId || !projectId || !viewDetails) return;

    updateView(workspaceSlug.toString(), teamspaceId.toString(), projectId.toString(), viewFilters);
  };
