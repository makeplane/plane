...
const TEMP_STORAGE_KEY = "sport-temp-create";
...
const SportProperty: React.FC<SportPropertyProps> = ({
  value,
  onChange,
  disabled = false,
  storageKey,
  ...
  const handleSelect = (sport: string | null) => {
    try {
      if (sport) localStorage.setItem(actualKey, JSON.stringify(sport));
      ...
      >
        {value ? (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs">Add Sport</span>
          </div>
        )}
      </div>
          ...
          {!loading &&
            ...
            filteredSports.map((sport) => (
              <div
                ...
                className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-custom-background-80"
              >
                <span className="text-xs whitespace-normal">{sport}</span>
              </div>
            ))}
        </div>
      )}
