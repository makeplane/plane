...
const TEMP_STORAGE_KEY = "level-temp-create";
...
const LevelProperty: React.FC<LevelPropertyProps> = ({
  value,
  onChange,
  disabled = false,
  storageKey,
  ...
  const handleSelect = (level: string | null) => {
    try {
      if (level) localStorage.setItem(actualKey, JSON.stringify(level));
          ...
          {!loading &&
            ...
            filteredLevels.map((level) => (
              <div
                ...
                className="flex items-center gap-2 px-2 py-1 cursor-pointer hover:bg-custom-background-80"
              >
                <span className="text-xs whitespace-normal">{level}</span>
              </div>
            ))}
        </div>
      )}
