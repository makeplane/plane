...
export const CapacityHeatmap = observer((props: ICapacityHeatmapProps) => {
    ...
    const days = (!dateFrom || !dateTo) ? [] : eachDayOfInterval({
            ...
            <div className="w-full overflow-x-auto custom-scrollbar">
                <table className="w-full min-w-max text-xs text-left border-collapse">
                    <thead className="bg-surface-2/80 backdrop-blur-sm uppercase text-[10px] font-bold text-tertiary border-b border-subtle sticky top-0 z-20">
                        <tr>
                            <th scope="col" className="px-3 py-2 sticky left-0 z-30 bg-surface-2 border-r border-subtle shadow-[1px_0_0_0_var(--color-border-subtle)]">
                                {t("capacity_member")}
                            </th>
                            ...
                            members.map((member) => {
                                ...
                                return (
                                    <tr key={member.member_id} className="group hover:bg-surface-2/50 transition-colors duration-200">
                                        ...
                                        <td className={`px-3 py-2 text-right font-bold ${isOverloaded ? "text-color-error" : "text-primary"
                                            }`}>
                                        ...
                                        {days.map((day) => {
                                            ...
                                            return (
                                                <td key={dateStr} className="px-1 py-1 text-center">
                                                    <Tooltip tooltipContent={cellInfo.tooltipKey ? t(cellInfo.tooltipKey) : undefined} disabled={!cellInfo.tooltipKey}>
                                                        <div className={`mx-auto flex h-8 w-[50px] items-center justify-center rounded-md border shadow-sm transition-all duration-300 hover:scale-[1.15] hover:shadow-md cursor-pointer ${cellInfo.className} font-medium text-[11px] tracking-wide`}>
                                                            {cellVal}
                                                        </div>
                                                    </Tooltip>
                                                </td>
                                        ...
                                        <td className="px-3 py-2 text-center border-l border-subtle/30">
                                            <div className="flex justify-center items-center">
                                                <Tooltip tooltipContent={t(tooltipKey)}>
                                                    <div className={`px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider ${statusBgClassName} ${statusTextClassName} ${statusBorderClassName}`}>
                                                        {isOverloaded ? t("capacity_overloaded", "Overload") :
                                                            isUnder ? t("capacity_under_capacity", "Under") :
                                                                member.total_estimated_minutes > 0 ? t("capacity_normal", "Normal") :
                                                                    "-"}
                                                    </div>
                                                </Tooltip>
                                            </div>
