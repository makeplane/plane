...
export function HoDepartmentTreeRow({ dept, depth = 0 }: Props) {
  ...
  return (
    <>
      <tr className="border-b border-subtle transition-colors odd:bg-surface-1 even:bg-surface-2 hover:bg-layer-2/50">
        <td className="py-2.5 pr-4" style={{ paddingLeft: `${depth * 20 + 12}px` }}>
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={() => setExpanded((v) => !v)}
              className={cn("h-4 w-4 flex-shrink-0 text-placeholder", !hasChildren && "invisible")}
            >
              <ChevronRight className={cn("h-4 w-4 transition-transform", expanded && "rotate-90")} />
            </button>
            <span className="text-13 font-medium text-primary">{dept.name}</span>
          </div>
        </td>
