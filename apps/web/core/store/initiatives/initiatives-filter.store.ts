...
export interface IInitiativeFilterStore {
  ...
  initInitiativeFilters: (workspaceSlug: string, isArchived?: boolean) => Promise<void>;
  updateDisplayFilters: (
    workspaceSlug: string,
    displayFilters: Partial<TInitiativeDisplayFilters>,
    isArchived?: boolean
  ) => void;
  updateFilters: (workspaceSlug: string, filters: TExternalInitiativeFilterExpression, isArchived?: boolean) => void;
}
...
export class InitiativeFilterStore implements IInitiativeFilterStore {
  ...
  initInitiativeFilters = async (workspaceSlug: string, isArchived = false) => {
    const userProperties = isArchived
      ? {
          display_filters: INITIATIVE_DEFAULT_DISPLAY_FILTERS,
          rich_filters: {},
        }
      : await this.initiativeService.fetchInitiativeUserProperties(workspaceSlug);
  ...
  updateDisplayFilters = (
    ...
    try {
      if (!isArchived)
        void this.initiativeService.updateInitiativeUserProperties(workspaceSlug, {
  ...
  updateFilters = (workspaceSlug: string, filters: TExternalInitiativeFilterExpression, isArchived = false) => {
    // Update the state
    runInAction(() => {
    ...
    void this.rootStore.initiativeStore.fetchFilteredInitiatives(workspaceSlug, filters, isArchived);
  };
}
