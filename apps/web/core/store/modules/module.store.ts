...
import type { ModuleMeta, ModulePermissions } from "./permissions";
import { ModulePermissionsInstance } from "./permissions";
...
export class ModulesStore implements IModuleStore {
  ...
  constructor(_rootStore: CoreRootStore) {
    ...
    // store
    this.permissions = new ModulePermissionsInstance({
      can: _rootStore.permissionAccessStore.can,
      getModuleMetaById: this.getModuleMetaById.bind(this),
    });
  ...
  // private helpers
  private getModuleMetaById = computedFn((moduleId: string): ModuleMeta | undefined => {
    const module = this.getModuleById(moduleId);
    if (!module) return undefined;
    const currentUserId = this.rootStore.user.data?.id;
    const conditionContext = { creator: !!(module.created_by && currentUserId && module.created_by === currentUserId) };
    return {
      conditionContext,
    };
  });
