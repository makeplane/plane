...
import type { CyclePermissions } from "./permissions";
import { CyclePermissionsInstance } from "./permissions";
...
export abstract class CycleStore implements ICycleStore {
  ...
  constructor(_rootStore: CoreRootStore) {
    ...
    // store
    this.permissions = new CyclePermissionsInstance(_rootStore, this);
  ...
  updateWorkspaceUserActiveCycleCount(workspaceSlug: string, increment: number) {
    this.rootStore.preferencesRoot.workspace.mutateActiveCyclesCount(workspaceSlug, increment);
  }
  ...
  updateCycleDetails = async (workspaceSlug: string, projectId: string, cycleId: string, data: Partial<ICycle>) => {
    try {
      ...
      console.error("Failed to patch cycle from cycle store", error);
      throw error;
    }
  };
