  ...
  const handleEscKeyDown = (event: KeyboardEvent) => {
    if (descriptionEditorRef.current?.isEditorReadyToDiscard()) {
      ...
      setToast({
        ...
        message: t("localized_ui.inbox.toasts.editor_processing"),
      });
  ...
  const handleFormSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setToast({
        ...
        message: t("localized_ui.inbox.toasts.create_success"),
      });
    } catch (error) {
      console.error(error);
      setToast({
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {duplicateIssues?.length > 0 && (
                <DeDupeButtonRoot
                  ...
                  label={t("localized_ui.inbox.duplicate_issues_found", { count: duplicateIssues.length })}
