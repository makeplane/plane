  ...
  return (
    <div className="horizontal-scroll-enable h-full w-full overflow-y-auto ">
      <table>
        <thead>
          <tr>
            <th className="h-11 min-w-[28rem] bg-surface-2 border-r border-subtle-1 animate-pulse" />
            {range(10).map((index) => (
              <th key={index} className="h-11 w-full min-w-[8rem] bg-surface-2 border-r border-subtle-1 animate-pulse" />
            ))}
          </tr>
        </thead>
        <tbody>
