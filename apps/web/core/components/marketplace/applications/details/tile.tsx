...
type TAppTileAction =
  ...
  | { type: "go_to_app" }
  | { type: "upgrade" }
  | { type: "not_configured"; tooltipMessage: string }
  | null;

type TAppTileConfig = {
  primaryCTA: TAppTileAction;
  showInstalledBadge: boolean;
  showInternalBadge: boolean;
  showOptionsMenu: boolean;
};

function getAppTileConfig(params: {
  ...
  const showInstalledBadge = !app.is_default && !!app.is_installed && isPaidOrOwned;
  ...
  if (isFreePlan && !app.is_owned) {
    primaryCTA = { type: "upgrade" };
  } else if (app.is_default && isNotSupported) {
    primaryCTA = { type: "not_configured", tooltipMessage };
  } else if (app.is_default) {
    primaryCTA = { type: "configure" };
  } else if (app.is_installed && app.configuration_url) {
    primaryCTA = { type: "configure" };
  } else if (app.setup_url && app.is_installed) {
    primaryCTA = { type: "go_to_app" };
  } else if (app.setup_url && !app.is_installed) {
    primaryCTA = { type: "install" };
  }

  return { primaryCTA, showInstalledBadge, showInternalBadge, showOptionsMenu };
}
...
export const AppTile = observer(function AppTile(props: AppTileProps) {
  ...
  const tooltipMessage = isSelfManaged
  ...
  const handleConfigure = () => {
    ...
    } else if (app.configuration_url) {
      router.push(`/${workspaceSlug}/${app.configuration_url}`);
    } else {
      setToast({
  ...
  const renderAction = () => {
    ...
    switch (cta.type) {
      ...
      case "not_configured":
        return (
          <Tooltip tooltipContent={cta.tooltipMessage}>
      ...
      case "go_to_app":
        return (
          <Button variant="link" className="p-0" onClick={handleInstall}>
