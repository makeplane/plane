...
export const DeleteRoleModal = observer(function DeleteRoleModal(props: Props) {
  ...
  // store hooks
  const { getRoleDetailsByRoleId, getWorkspaceRolesByWorkspaceSlug, getProjectRolesByWorkspaceSlug, deleteRole } =
    useRoleManagement();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setToast({
        ...
        title: t("success"),
        message: t("workspace_settings.settings.roles_and_schemes.delete_role.success_toast_message"),
      });
    } catch {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("error"),
        message: t("workspace_settings.settings.roles_and_schemes.delete_role.error_toast_description"),
      });
    }
  };
    ...
    <ModalCore isOpen={isOpen} handleClose={onClose} position={EModalPosition.CENTER} width={EModalWidth.XL}>
      ...
      <div className="flex items-center justify-between px-5 pt-5">
        <h4 className="text-h4-medium text-secondary">
          {t("workspace_settings.settings.roles_and_schemes.delete_role.confirm_title", { roleName })}
        </h4>
      ...
      <div className="px-5 py-4 space-y-4">
        <p className="text-body-sm-regular text-secondary">
          {hasMembersToReassign
            ? t("workspace_settings.settings.roles_and_schemes.delete_role.has_members_message", {
                count: memberCount,
              })
            : t("workspace_settings.settings.roles_and_schemes.delete_role.no_members_message")}
        </p>
        {hasMembersToReassign && (
          <div className="flex flex-col gap-y-2">
            <label htmlFor="reassignRole" className="text-body-sm-medium text-secondary">
              {t("workspace_settings.settings.roles_and_schemes.delete_role.reassign_label")}
            ...
            <CustomSelect
              ...
              label={
                <span className={`text-13 ${!selectedRole ? "text-placeholder" : ""}`}>
                  {selectedRole?.name ??
                    t("workspace_settings.settings.roles_and_schemes.delete_role.reassign_placeholder")}
                </span>
              }
      ...
      <div className="flex items-center justify-end gap-3 border-t border-subtle px-5 py-4">
        ...
        <Button variant="error-fill" onClick={handleDelete} disabled={isSubmitDisabled}>
          {hasMembersToReassign
            ? t("workspace_settings.settings.roles_and_schemes.delete_role.submit_button")
            : t("workspace_settings.settings.roles_and_schemes.delete_role.submit_button_no_members")}
        </Button>
      </div>
    </ModalCore>
  );
