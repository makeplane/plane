...
type TProps = {
  ...
  isDisabled?: boolean;
  /** When the automation trigger is fixed schedule, only run script is a valid action. */
  isTimeBasedTrigger?: boolean;
};

export function AutomationActionHandlerDropdown(props: TProps) {
  const { value, onChange, isDisabled, isTimeBasedTrigger } = props;
  ...
  const filteredOptions = AUTOMATION_ACTION_HANDLER_OPTIONS.filter((option) => {
    if (isTimeBasedTrigger && option.value !== EActionNodeHandlerName.RUN_SCRIPT) return false;
    return !option.integrationKey || enabledIntegrations.includes(option.integrationKey as E_INTEGRATION_KEYS);
  });
