...
import type { TDateAttributeDisplayOptions, CustomProperty, TOperationMode } from "@plane/types";
...
type TDatePickerAttributesProps = {
  datePickerPropertyDetail: Partial<CustomProperty<"DATETIME">>;
  currentOperationMode: TOperationMode;
  onDatePickerDetailChange: <K extends keyof CustomProperty<"DATETIME">>(
    key: K,
    value: CustomProperty<"DATETIME">[K],
    shouldSync?: boolean
  ) => void;
    ...
    <>
      {ISSUE_PROPERTY_SETTINGS_CONFIGURATIONS?.DATETIME?.map((configurations, index) => (
        <PropertySettingsConfiguration
          key={index}
          settings={datePickerPropertyDetail.settings ?? undefined}
          settingsConfigurations={configurations}
          onChange={(value) => onDatePickerDetailChange("settings", value as CustomProperty<"DATETIME">["settings"])}
