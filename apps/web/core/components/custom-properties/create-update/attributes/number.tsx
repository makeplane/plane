...
type TNumberAttributesProps = {
  numberPropertyDetail: Partial<CustomProperty<"DECIMAL">>;
  currentOperationMode: TOperationMode;
  onNumberDetailChange: <K extends keyof CustomProperty<"DECIMAL">>(
    key: K,
    value: CustomProperty<"DECIMAL">[K],
    shouldSync?: boolean
  ) => void;
        ...
        <div className="pb-2">
          {ISSUE_PROPERTY_SETTINGS_CONFIGURATIONS?.DECIMAL?.map((configurations, index) => (
            <PropertySettingsConfiguration
              key={index}
              settings={numberPropertyDetail.settings ?? undefined}
              settingsConfigurations={configurations}
              onChange={(value) => onNumberDetailChange("settings", value as CustomProperty<"DECIMAL">["settings"])}
