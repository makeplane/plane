...
// local imports
// TODO: <WorkspaceWorkItemType> Port value component
...
type TBooleanAttributesProps = {
  booleanPropertyDetail: Partial<CustomProperty<"BOOLEAN">>;
  currentOperationMode: TOperationMode;
  onBooleanDetailChange: <K extends keyof CustomProperty<"BOOLEAN">>(
    key: K,
    value: CustomProperty<"BOOLEAN">[K],
    shouldSync?: boolean
  ) => void;
        ...
        <div className="pb-4">
          {ISSUE_PROPERTY_SETTINGS_CONFIGURATIONS?.BOOLEAN?.map((configurations, index) => (
            <PropertySettingsConfiguration
              key={index}
              settings={booleanPropertyDetail.settings ?? undefined}
              settingsConfigurations={configurations}
              onChange={(value) => onBooleanDetailChange("settings", value as CustomProperty<"BOOLEAN">["settings"])}
