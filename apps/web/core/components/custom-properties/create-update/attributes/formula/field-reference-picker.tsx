...
type TFieldReferencePickerProps = {
  ...
  onSelect: (property: CustomProperty<CustomPropertyType>) => void;
  onClose: () => void;
  initialSearchQuery?: string;
};

// Get icon for property type
const getPropertyIcon = (propertyType: CustomPropertyType) => {
  switch (propertyType) {
    case "DATETIME":
      return Calendar;
    case "DECIMAL":
...
export const FieldReferencePicker = observer(function FieldReferencePicker(props: TFieldReferencePickerProps) {
  ...
  const filteredProperties = useMemo(() => {
    const filtered = properties.filter((p) => {
      ...
      if (!isAllowedFormulaPropertyType(p.property_type as any)) return false;
