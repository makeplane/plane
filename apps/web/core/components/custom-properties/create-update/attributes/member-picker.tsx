...
type TMemberPickerAttributesProps = {
  memberPickerPropertyDetail: Partial<CustomProperty<"RELATION">>;
  currentOperationMode: TOperationMode;
  onMemberPickerDetailChange: <K extends keyof CustomProperty<"RELATION">>(
    key: K,
    value: CustomProperty<"RELATION">[K],
    shouldSync?: boolean
  ) => void;
        ...
        <div className="pt-4">
          {ISSUE_PROPERTY_SETTINGS_CONFIGURATIONS?.RELATION_USER?.map((configurations, index) => (
            <PropertySettingsConfiguration
              key={index}
              settings={memberPickerPropertyDetail.settings ?? undefined}
              settingsConfigurations={configurations}
              onChange={(value) =>
                onMemberPickerDetailChange("settings", value as CustomProperty<"RELATION">["settings"])
              }
