...
const CyclesInsightTable = observer(function CyclesInsightTable() {
    ...
    const { data: cyclesData, isLoading } = useSWR(
        `insights-table-cycles-${workspaceSlug}-${selectedDuration}-${selectedProjects}-${selectedCycle}-${selectedModule}-${isPeekView}-${isEpic}`,
        () =>
    ...
    const columnsLabels: Record<keyof Omit<CycleInsightColumns, "id" | "project_id">, string> = useMemo(
        () => ({
            name: t("common.cycle"),
    ...
    const columns: ColumnDef<AnalyticsTableDataMap["cycles"]>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => <div className="text-right">{renderFormattedDate(new Date(row.original.end_date))}</div>,
                meta: {
                    export: {
                        key: columnsLabels["end_date"],
                        value: (row) => renderFormattedDate(new Date(row.original.end_date)) ?? "",
                    },
                },
            },
            {
