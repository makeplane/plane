...
const CyclesDistribution = observer(function CyclesDistribution() {
    const {
        selectedDuration,
    ...
    const { data: cyclesDistributionData, isLoading } = useSWR(
        `cycles-distribution-${workspaceSlug}-${selectedDuration}-${selectedProjects}-${selectedCycle}-${selectedModule}-${isPeekView}-${isEpic}`,
        () =>
    ...
    const parsedData = useMemo(() => {
        if (!cyclesDistributionData?.data) return [];
        return cyclesDistributionData.data.map((datum: any) => ({
            ...datum,
            pending_issues: datum.total_issues - datum.completed_issues,
        }));
    }, [cyclesDistributionData]);
    ...
    return (
        <AnalyticsSectionWrapper
            title={t("workspace_analytics.tabs.cycle_progress")}
...
export default CyclesDistribution;
