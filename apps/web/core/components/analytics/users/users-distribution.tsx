...
const UsersDistribution = observer(function UsersDistribution() {
    const {
        selectedDuration,
    ...
    const { data: usersDistributionData, isLoading } = useSWR(
        `users-distribution-${workspaceSlug}-${selectedDuration}-${selectedProjects}-${selectedCycle}-${selectedModule}-${isPeekView}-${isEpic}`,
        () =>
    ...
    const parsedData = useMemo(() => {
        if (!usersDistributionData?.data) return [];
        return usersDistributionData.data;
    ...
    return (
        <AnalyticsSectionWrapper
            title={t("workspace_analytics.tabs.work_items_resolved_vs_pending")}
