  ...
  return invitations && invitations.length > 0 ? (
    <div className="space-y-4">
      <div className="mx-auto space-y-1 py-4 text-center">
        <h3 className="text-24 font-bold text-primary">{t("localized_ui.onboarding.you_are_invited")}</h3>
        <p className="font-medium text-placeholder">{t("localized_ui.onboarding.accept_invites")}</p>
      </div>
      <div>
        {invitations &&
      ...
      >
        {isJoiningWorkspaces ? (
          ...
          t("localized_ui.onboarding.continue_to_workspace")
        )}
      </Button>
      <div className="mx-auto mt-4 flex items-center sm:w-96">
        <hr className="w-full border-strong" />
        <p className="mx-3 flex-shrink-0 text-center text-13 text-placeholder">{t("localized_ui.onboarding.or")}</p>
        <hr className="w-full border-strong" />
      </div>
      <Button
        variant="ghost"
      ...
      >
        {t("localized_ui.onboarding.create_own_workspace")}
      </Button>
    </div>
  ) : (
    <div>{t("localized_ui.onboarding.no_invitations_found")}</div>
  );
}
