    ...
    <div>
        ...
        <div className="col-span-6">
          <Controller
            ...
            rules={{
              pattern: {
                value: emailRegex,
                message: t("localized_ui.onboarding.invalid_email"),
              },
            }}
      ...
      {email && !emailRegex.test(email) && (
        <div className="mx-8 my-1">
          ...
          <span className="mt-1 text-11 text-danger-primary">{t("localized_ui.onboarding.invalid_email")}</span>
        </div>
      )}
    </div>
  );
    ...
    await workspaceService
      ...
      .then(() => {
        setToast({
          ...
          message: t("localized_ui.onboarding.invitations_sent"),
        });
        return nextStep();
      })
      .catch((err) => {
  ...
  return (
    ...
    >
      <CommonOnboardingHeader
        title={t("localized_ui.onboarding.invite_team_title")}
        description={t("localized_ui.onboarding.invite_team_description")}
      />
      <div className="w-full py-4 text-13">
        <div className="group relative mx-8 grid grid-cols-10 gap-4 py-2">
          <div className="col-span-6 px-1 text-13 font-medium text-secondary">{t("localized_ui.space_auth.email")}</div>
        ...
        >
          ...
          {t("localized_ui.onboarding.add_another")}
        </button>
      </div>
      <div className="mx-auto flex w-full flex-col items-center justify-center gap-4 px-8 sm:px-2">
        ...
        >
          {isSubmitting ? <Spinner height="20px" width="20px" /> : t("localized_ui.onboarding.continue")}
        </Button>
        <Button variant="ghost" size="xl" className="w-full" onClick={nextStep}>
          {t("localized_ui.onboarding.do_later")}
        </Button>
      </div>
    </form>
  );
