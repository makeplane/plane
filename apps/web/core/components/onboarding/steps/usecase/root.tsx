...
export const UseCaseSetupStep = observer(function UseCaseSetupStep({ handleStepChange }: Props) {
  ...
  const handleSubmitUserPersonalization = async (formData: TProfileSetupFormValues) => {
    ...
    try {
      ...
      setToast({
        ...
        message: t("onboarding.profile_setup_completed"),
      });
    } catch {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("onboarding.error"),
        message: t("onboarding.profile_setup_failed"),
      });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-10">
      {/* Header */}
      <CommonOnboardingHeader title={t("onboarding.usecase_title")} description={t("onboarding.usecase_description")} />

      {/* Use Case Selection */}
      <div className="flex flex-col gap-3">
        <p className="text-body-sm-semibold text-placeholder">{t("onboarding.select_one_or_more")}</p>

        <Controller
          control={control}
          name="use_case"
          rules={{
            required: t("onboarding.select_at_least_one"),
            validate: (value) => (value && value.length > 0) || t("onboarding.select_at_least_one"),
          }}
