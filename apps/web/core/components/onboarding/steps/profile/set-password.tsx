    ...
    <div className={`flex flex-col overflow-hidden rounded-lg bg-surface-2 transition-all duration-300 ease-in-out`}>
      ...
      >
        <div className="flex items-center gap-1 text-tertiary">
          ...
          <span className="font-medium">{t("onboarding.set_password")}</span>
          <span>({t("onboarding.optional")})</span>
        </div>
      ...
      <div className={expandedContentClasses}>
        ...
        <div className="flex flex-col gap-2 pb-2">
          ...
          <div className="transform transition-all delay-100 duration-300 ease-in-out">
            ...
            />
            {hasPasswordMismatch && (
              <p className="mt-1 text-11 text-danger-primary">{t("onboarding.passwords_do_not_match")}</p>
            )}
            {isPasswordValid && (
              <p className="mt-1 text-11 text-success-primary">✓ {t("onboarding.passwords_match")}</p>
            )}
          </div>
        </div>
      </div>
