...
export const ReporterOptions = observer(function ReporterOptions(props: Props) {
  const {
    getUserDetails,
    isOpen,
    memberIds,
  ...
  const isValidEmailFormat = query.includes("@") && query.length > 5;
  const isWinjitDomain = query.endsWith("@winjit.com");
  const showCustomOption = query !== "" && !filteredOptions?.find(o => o?.query === query);
      ...
      >
        <div className="flex items-center gap-1.5 rounded-sm border border-subtle bg-surface-2 px-2">
          ...
          <Combobox.Input
            ...
            placeholder={"Search or enter @winjit.com email"}
            displayValue={() => ""}
        ...
        <div className="mt-2 max-h-48 space-y-1 overflow-y-scroll">
          {showCustomOption && (
            <Combobox.Option
              ...
              className={({ active, selected }) =>
                cn(
                  "flex w-full items-center justify-between gap-2 truncate rounded-sm px-1 py-1.5 select-none cursor-pointer",
            ...
            >
              {({ selected }) => (
                <>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded-full bg-orange-400/20 text-orange-600 flex items-center justify-center font-medium text-[10px] ring-2 ring-orange-400/60 ring-offset-1">
                      @
                    </div>
                    <span className="flex-grow truncate">
                      {isWinjitDomain ? `Use "${query}"` : "Email must end in @winjit.com"}
                    </span>
                  </div>
          ...
          {filteredOptions && filteredOptions.length > 0 && (
             <div className="py-1">
               {showCustomOption && <div className="border-t border-subtle my-1"></div>}
               {filteredOptions.map(
                (option) =>
                  option && (
                    <Combobox.Option
          ...
          {!showCustomOption && (!filteredOptions || filteredOptions.length === 0) && (
