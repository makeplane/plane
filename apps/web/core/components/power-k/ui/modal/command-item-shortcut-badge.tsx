    ...
    <div className="shrink-0 pointer-events-none inline-flex items-center gap-1 select-none font-medium">
      {formatted?.split("").map((char, index) => (
        <React.Fragment key={index}>
          <kbd className="inline-flex h-5 items-center justify-center rounded border border-strong bg-custom-background-100 px-1.5 font-mono text-10 font-medium text-tertiary">
            {char.toUpperCase()}
          </kbd>
        </React.Fragment>
      ))}
