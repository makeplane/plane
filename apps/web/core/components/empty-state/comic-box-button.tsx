    ...
    <Popover as="div" className="relative">
      ...
      {isHovered && (
        <Popover.Panel className="fixed z-10" static>
          <div
            className="flex flex-col rounded-sm border border-subtle-1 bg-surface-1 p-5 relative w-52 lg:w-60 xl:w-80"
            ref={setPopperElement as Ref<HTMLDivElement>}
            style={styles.popper}
            {...attributes.popper}
          >
            <div className="absolute w-2 h-2 bg-surface-1 border rounded-lb-sm  border-subtle-1 border-r-0 border-t-0 transform rotate-45 bottom-2 -left-[5px]" />
