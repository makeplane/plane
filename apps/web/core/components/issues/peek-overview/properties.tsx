      ...
      <div className={`mt-3 w-full space-y-3 ${disabled ? "opacity-60" : ""}`}>
        ...
        <SidebarPropertyListItem icon={MembersPropertyIcon} label={t("common.reviewers")}>
          <MemberDropdown
            ...
            className="group w-full grow"
            buttonContainerClassName="w-full text-left h-7.5"
            buttonClassName={`text-body-xs-medium justify-between ${issue?.reviewer_ids?.length > 0 ? "" : "text-placeholder"}`}
        ...
        <SidebarPropertyListItem icon={DropdownPropertyIcon} label={t("common.product")}>
          <CustomSelect
            ...
            label={
              <span className={`text-body-xs-medium ${issue?.product ? "" : "text-placeholder"}`}>
