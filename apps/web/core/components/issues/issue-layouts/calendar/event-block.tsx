...
export const CalendarEventBlock: React.FC<Props> = observer(
  ({ issue, quickActions, canEditProperties, isEpic = false, style }) => {
    ...
    const handleClick = (e: React.MouseEvent) => {
      ...
      const url = `/spip/projects/${issue.project_id}/${isEpic ? "epics" : "issues"}/${issue.id}`;
      router.push(url);
    };
    ...
    return (
      ...
      >
        <div className="h-full p-2 flex flex-col gap-1">
          ...
          {/* Issue title */}
          <div className="text-xs font-medium text-custom-text-100 line-clamp-2">{issue.name}</div>

          {/* Issue metadata */}
          {/* <div className="flex items-center gap-2 text-[10px] text-custom-text-300 mt-auto">
            {issue.project_id && <span className="truncate">{issue.project_id}</span>}
            {issue.sequence_id && <span className="font-mono">#{issue.sequence_id}</span>}
          </div> */}
