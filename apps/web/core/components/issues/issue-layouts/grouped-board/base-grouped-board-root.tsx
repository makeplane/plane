...
export type GroupedBoardStoreType =
...
export interface IBaseGroupedBoardLayout {
...
export const BaseGroupedBoardRoot = observer(function BaseGroupedBoardRoot(props: IBaseGroupedBoardLayout) {
  const {
    QuickActions,
    addIssuesToView,
  ...
  // store hooks
  const storeType = useIssueStoreType() as GroupedBoardStoreType;
  ...
  // Enable Auto Scroll for Main Board
  useEffect(() => {
  ...
  return (
    <>
      ...
      <IssueLayoutHOC layout={EIssueLayoutTypes.GROUPED_BOARD}>
        <div
          className={`horizontal-scrollbar relative flex scrollbar-lg h-full w-full ${sub_group_by ? "vertical-scrollbar overflow-y-auto" : "overflow-x-auto overflow-y-hidden"}`}
