...
import { KanbanIssueBlocksList } from "../kanban/blocks-list";

interface IGroupedBoardColumn {
...
export const GroupedBoardColumn = observer(function GroupedBoardColumn(props: IGroupedBoardColumn) {
  const {
    column,
    columnIndex,
    sub_group_id,
  ...
  return (
    <div
      ...
      className={cn(
        "flex-shrink-0 w-[220px] min-h-[150px] p-2",
        "bg-surface-2 rounded-b-lg",
        { "bg-surface-3": isDraggingOverColumn && !shouldOverlayBeVisible }
      )}
      ref={columnRef}
    >
      <GroupDragOverlay
      ...
      {enableQuickIssueCreate &&
        ...
        !getIsWorkflowWorkItemCreationDisabled(groupId, sub_group_id) && (
          <div className="sticky bottom-0 w-full py-0.5 opacity-0 hover:opacity-100 focus-within:opacity-100 transition-opacity">
            <QuickAddIssueRoot
              layout={EIssueLayoutTypes.GROUPED_BOARD}
