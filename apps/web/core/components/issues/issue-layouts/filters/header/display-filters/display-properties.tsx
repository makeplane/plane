...
// plane web helpers
import { AdditionalDisplayProperties } from "@/plane-web/components/issues/issue-layouts/additional-display-properties";
// components
...
export const FilterDisplayProperties = observer(function FilterDisplayProperties(props: Props) {
  ...
  const getDisplayPropertyLabelKey = (displayProperty: (typeof ISSUE_DISPLAY_PROPERTIES)[number]) => {
    if (isEpic && displayProperty.key === "sub_issue_count") {
      return "issue.display.properties.work_item_count";
    }
    return displayProperty.titleTranslationKey;
  };
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          {filteredDisplayProperties.map((displayProperty) => (
            <>
              ...
              >
                {t(getDisplayPropertyLabelKey(displayProperty))}
              </button>
            </>
          ))}
