...
export const FilterStartDate = observer(function FilterStartDate(props: Props) {
  ...
  return (
    <>
      ...
      <FilterHeader
        title={`Дата начала${appliedFiltersCount > 0 ? ` (${appliedFiltersCount})` : ""}`}
        ...
        <div>
          {filteredOptions.length > 0 ? (
            <>
              ...
              <FilterOption isChecked={isCustomDateSelected()} onClick={handleCustomDate} title="Произвольная" multiple />
            </>
          ) : (
            <p className="text-11 text-placeholder italic">Ничего не найдено</p>
          )}
        </div>
      )}
    </>
