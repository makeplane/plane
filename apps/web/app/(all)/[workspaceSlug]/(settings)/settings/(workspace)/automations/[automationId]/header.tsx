...
export const WorkspaceAutomationDetailsHeader = observer(function WorkspaceAutomationDetailsHeader(props: TProps) {
  const { automationId, workspaceSlug } = props;
  // router
  ...
  const {
    getAutomationById,
    workspaceAutomations: { getWorkspaceAutomationIds, deleteAutomation },
  } = useAutomations();
  // derived values
  ...
  const switcherOptions = getWorkspaceAutomationIds(workspaceSlug)
    ?.map((id) => {
  ...
  return (
    <Header>
      <Header.LeftItem>
        <Breadcrumbs onBack={() => router.back()} isLoading={!automationDetails}>
          <Breadcrumbs.Item
            component={
              <BreadcrumbLink
          ...
          <Breadcrumbs.Item
            component={
              <BreadcrumbNavigationSearchDropdown
                ...
                icon={
                  <Breadcrumbs.Icon>
                    <Repeat className="size-4 shrink-0 text-tertiary" />
                  </Breadcrumbs.Icon>
                }
                isLast
              />
      ...
      <Header.RightItem className="items-center">
        ...
        <AutomationQuickActions
          ...
          deleteAutomation={deleteAutomation.bind(deleteAutomation, workspaceSlug)}
        />
      </Header.RightItem>
    </Header>
  );
