...
function ProjectSettingsPage() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-[480px] flex-col items-center justify-center gap-4">
      <img src={resolvedPath} alt={t("projects_empty_state.no_projects_yet")} />
      <div className="text-16 font-semibold text-tertiary">{t("projects_empty_state.no_projects_yet")}</div>
      <div className="text-center text-13 text-tertiary">{t("projects_empty_state.description")}</div>
      <div className="flex gap-2">
        <Link href="https://plane.so/" target="_blank" className={cn(getButtonStyling("secondary", "base"))}>
          {t("projects_empty_state.learn_more")}
        </Link>
        <Button
          onClick={() => toggleCreateProjectModal(true)}
          data-ph-element={PROJECT_TRACKER_ELEMENTS.EMPTY_STATE_CREATE_PROJECT_BUTTON}
        >
          {t("projects_empty_state.start_first_project")}
        </Button>
      </div>
    </div>
  );
