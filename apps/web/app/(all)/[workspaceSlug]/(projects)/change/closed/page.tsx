...
const ClosedChangesPage = observer(() => {
  const { workspaceSlug } = useParams();
  const store = useChangeManagement();
  const [filters, setFilters] = useState<IChangeFilters>({ view: "closed" });
  ...
  const handleFilterChange = (newFilters: IChangeFilters) => {
    setFilters({ ...newFilters, view: "closed" });
  };
...
export default ClosedChangesPage;
