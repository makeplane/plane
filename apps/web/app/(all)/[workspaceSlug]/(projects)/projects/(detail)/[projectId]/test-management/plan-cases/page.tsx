...
export default function PlanCasesPage() {
  ...
  useEffect(() => {
    if (!repositoryId && workspaceSlug) {
      ...
      const current = `/${ws}/projects/${pid}/test-management/plan-cases${searchParams.toString() ? `?${searchParams.toString()}` : ""}`;
      try {
      ...
      render: (_: any, record: PlanCaseItem) => {
        ...
        return (
          <Button
            ...
            onClick={() =>
              router.push(
                `/${workspaceSlug}/projects/${pid}/test-management/test-execution?case_id=${encodeURIComponent(String(cid))}&plan_id=${encodeURIComponent(String(planId || ""))}${repoQuery}`
              )
            }
          >
            {name}
