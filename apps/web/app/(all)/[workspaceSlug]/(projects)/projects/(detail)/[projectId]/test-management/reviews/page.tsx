...
export default function ReviewsPage() {
  ...
  useEffect(() => {
    if (!repositoryId && workspaceSlug) {
      ...
      const current = `/${ws}/projects/${pid}/test-management/reviews${searchParams.toString() ? `?${searchParams.toString()}` : ""}`;
  ...
  const columns: TableProps<ReviewItem>["columns"] = [
    {
      ...
      render: (name: string, record: ReviewItem) => (
        <Button
          ...
          onClick={() =>
            router.push(
              `/${workspaceSlug}/projects/${pid}/test-management/caseManagementReviewDetail?review_id=${record.id}`
            )
          }
        >
          {name}
