...
from workspaces.utils.ip_address import validate_url
from workspaces.bgtasks.work_item_link_task import fetch_and_encode_favicon, DEFAULT_FAVICON
from workspaces.authentication.adapter.base import Adapter
...
class TestWebhookUrlValidation:
    ...
    def test_webhook_url_to_internal_is_rejected(self, ip):
        with patch("workspaces.utils.ip_address.socket.getaddrinfo") as dns:
...
class TestWebhookPatchContextGuard:
    def _serializer_with_request(self, host):
        from workspaces.app.serializers import WebhookSerializer
...
class TestWebhookRebinding:
    @patch("workspaces.utils.url_security.requests.Session")
    @patch("workspaces.utils.url_security.resolve_and_validate")
    def test_connection_pinned_to_validated_ip(self, mock_resolve, mock_session_cls):
        from workspaces.utils.url_security import pinned_fetch
...
class TestFaviconRedirect:
    @patch("workspaces.utils.url_security.requests.Session")
    @patch("workspaces.utils.url_security.resolve_and_validate")
    @patch("workspaces.bgtasks.work_item_link_task.socket.getaddrinfo")
...
class TestOAuthAvatarSSRF:
    ...
    @patch("workspaces.authentication.adapter.base.pinned_fetch_following_redirects")
