...
from workspaces.db.models import Project, ProjectMember, State, User, WorkspaceMember
...
class TestProjectListCreateAPIEndpoint:
    ...
    def test_model_activity_not_called_on_rollback(self, api_key_client, workspace, create_user):
        ...
        with (
            mock.patch(
                "workspaces.api.views.project.State.objects.bulk_create",
                side_effect=forced_error,
            ),
            mock.patch("workspaces.api.views.project.model_activity") as mocked_activity,
        ):
            response = api_key_client.post(url, payload, format="json")
    ...
    def test_response_still_201_when_broker_dispatch_fails(self, api_key_client, workspace, create_user):
        ...
        with mock.patch("workspaces.api.views.project.model_activity") as mocked_activity:
