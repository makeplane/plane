...
class ProjectEstimateAPIEndpoint(BaseAPIView):
    ...
    required_alternate_scopes = {
        ...
        "PATCH": [[WRITE_SCOPE], [PROJECTS_ESTIMATES_WRITE_SCOPE]],
        "DELETE": [[WRITE_SCOPE], [PROJECTS_ESTIMATES_WRITE_SCOPE]],
    }
    ...
    def post(self, request, slug, project_id):
        ...
        # check if the external estimate already exists
        if external_id and external_source:
            existing_external_estimate = self.model.objects.filter(
                external_id=external_id, external_source=external_source
            ).first()
            if existing_external_estimate:
                # return 409 if the external estimate already exists
                return Response(
                    status=status.HTTP_409_CONFLICT,
                    data={
                        "error": "Estimate with the same external id and external source already exists",
                        "id": str(existing_external_estimate.id),
                    },
                )
...
class EstimatePointListCreateAPIEndpoint(BaseAPIView):
    ...
    def post(self, request, slug, project_id, estimate_id):
        ...
        external_pairs = []
        seen = set()
        for item in estimate_points_data:
            ext_id = item.get("external_id")
            ext_source = item.get("external_source")
            if ext_id and ext_source:
                key = (ext_id, ext_source)
                if key in seen:
                    return Response(
                        status=status.HTTP_409_CONFLICT,
                        data={
                            "error": "Duplicate external id and external source in request",
                        },
                    )
                seen.add(key)
                external_pairs.append((ext_id, ext_source))

        if external_pairs:
            q_filter = Q()
            for ext_id, ext_source in external_pairs:
                q_filter |= Q(external_id=ext_id, external_source=ext_source)
            existing = self.get_queryset().filter(q_filter).first()
            if existing:
                return Response(
                    status=status.HTTP_409_CONFLICT,
                    data={
                        "error": "Estimate point with the same external id and external source already exists",
                        "id": str(existing.id),
                    },
                )
...
class EstimatePointDetailAPIEndpoint(BaseAPIView):
    ...
    def patch(self, request, slug, project_id, estimate_id, estimate_point_id):
        ...
        ALLOWED_FIELDS = ["key", "value", "description", "external_id", "external_source"]
        filtered_data = {k: v for k, v in request.data.items() if k in ALLOWED_FIELDS}
        if not filtered_data:
            return Response(self.serializer_class(estimate_point).data, status=status.HTTP_200_OK)
        # check for external id and external source
        if filtered_data.get("external_id") and filtered_data.get("external_source"):
            existing = (
                self.get_queryset()
                .filter(
                    external_id=filtered_data.get("external_id"),
                    external_source=filtered_data.get("external_source"),
                )
                .first()
            )
            if existing:
