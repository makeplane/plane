...
def validate_html_content(html_content):
    ...
    malicious_patterns = [
        ...
        r'<iframe[^>]*src\s*=\s*["\']?(?!(?:https?://(?:www\.)?(?:youtube\.com|vimeo\.com|codepen\.io)|about:blank))',
    ...
    try:
        ...
        # Check for some self-closing tags that don't need closing
        self_closing_tags = {
            "img",
            "br",
            "hr",
...
def _validate_json_content_array(content):
    """
    ...
    for node in content:
        if isinstance(node, dict):
            ...
            if "content" in node and isinstance(node["content"], list):
                is_valid, error_msg = _validate_json_content_array(node["content"])
                if not is_valid:
                    return False, error_msg
...
def _validate_json_content_recursive(obj):
    """
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            if isinstance(value, str):
                # More targeted dangerous patterns
                dangerous_patterns = [
                    r"<script[^>]*>.*?</script>",
                    r"javascript\s*:",
                    r"data\s*:\s*text/html",
                ...
                is_valid, error_msg = _validate_json_content_recursive(value)
                if not is_valid:
                    return False, error_msg
    elif isinstance(obj, list):
        for item in obj:
            is_valid, error_msg = _validate_json_content_recursive(item)
            if not is_valid:
                return False, error_msg
