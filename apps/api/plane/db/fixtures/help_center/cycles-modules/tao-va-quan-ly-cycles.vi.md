...
Tạo Cycle để chia công việc của dự án thành các giai đoạn có ngày bắt đầu và kết thúc rõ ràng — giúp nhóm tập trung hoàn thành một lượng công việc cụ thể trong mỗi giai đoạn và theo dõi tiến độ qua biểu đồ burndown/burnup.
...
2. Nhấn nút **+ Cycle** (góc trên phải) hoặc **Add Cycle** trong tab tương ứng.
3. Nhập thông tin trong hộp thoại:
   - **Tên** — bắt buộc (ví dụ: "Sprint 12 — Tháng 6/2025").
   - **Mô tả** — tuỳ chọn.
   - **Ngày bắt đầu** và **Ngày kết thúc** — chọn bằng bộ chọn ngày; hai trường này quyết định Cycle thuộc tab nào (Active / Upcoming / Completed).
4. Nhấn **Save** để tạo.
...
1. Nhấn **+ Add work items** (thanh trên cùng trong Cycle) để thêm công việc hiện có.
2. Tìm kiếm theo tên hoặc mã công việc (ví dụ: SHB-123), chọn và nhấn **Add**.
3. Hoặc tạo công việc mới ngay trong Cycle bằng **+ New work item** trong bố cục List/Board.

{{screenshot:tao-va-quan-ly-cycles}}

### Chuyển công việc chưa xong sang Cycle mới (Transfer)

Khi một Cycle kết thúc mà còn công việc chưa hoàn thành:

1. Trên thẻ Cycle đã kết thúc, nhấn biểu tượng **ba chấm (⋯)** → **Transfer Issues**.
...
| Hành động            | Mô tả                                   |
| -------------------- | --------------------------------------- |
| **Edit**             | Sửa tên, mô tả, ngày                    |
| **Copy Link**        | Sao chép đường dẫn trực tiếp            |
| **Open in new tab**  | Mở Cycle trong tab trình duyệt mới      |
| **Add to Favorites** | Ghim vào danh sách yêu thích            |
| **Archive**          | Lưu trữ Cycle (ẩn khỏi danh sách chính) |
| **Delete**           | Xoá vĩnh viễn (không thể khôi phục)     |
...
- Mỗi thời điểm chỉ có **một Cycle active** (ngày hiện tại nằm trong khoảng start–end). Nếu tạo Cycle có ngày trùng với Cycle đang active, hệ thống sẽ cảnh báo xung đột.
