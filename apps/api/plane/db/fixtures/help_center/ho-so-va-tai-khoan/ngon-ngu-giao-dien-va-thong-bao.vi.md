---
category: ho-so-va-tai-khoan
slug: ngon-ngu-giao-dien-va-thong-bao
sort_order: 30000
title: "Ngôn ngữ, giao diện & thông báo"
status: published
---

## Mục đích

Trang này cho phép bạn chọn ngôn ngữ hiển thị, múi giờ, ngày đầu tuần, giao diện sáng/tối và bật/tắt từng loại thông báo email theo sự kiện công việc.

## Các bước

### Mở trang Tùy chọn

1. Nhấn **ảnh đại diện** → **Cài đặt tài khoản**.
2. Chọn tab **Tùy chọn** _(Preferences)_ trong menu bên trái.

{{screenshot:ngon-ngu-giao-dien-va-thong-bao}}

---

### Đổi giao diện (Sáng / Tối / Tương phản)

1. Trong mục **Giao diện**, nhấn vào bộ chọn giao diện hiện tại.
2. Chọn một trong các tùy chọn:

   | Tùy chọn                               | Mô tả                                                  |
   | -------------------------------------- | ------------------------------------------------------ |
   | **Sáng** _(Light)_                     | Nền trắng, chữ tối — mặc định                          |
   | **Tối** _(Dark)_                       | Nền tối, giảm mỏi mắt ban đêm                          |
   | **Sáng tương phản** _(Light Contrast)_ | Tương phản cao cho người dùng cần khả năng đọc tốt hơn |
   | **Tối tương phản** _(Dark Contrast)_   | Tương phản cao kết hợp nền tối                         |
   | **Tùy chỉnh** _(Custom)_               | Tự chọn màu chính và màu nền                           |

3. Trang tải lại tự động để áp dụng giao diện mới.

> Chọn **Tùy chỉnh** sẽ mở thêm bộ chọn màu để bạn tự tùy chỉnh màu nhấn và màu nền.

{{screenshot:giao-dien-theme-switcher}}

---

### Đổi ngôn ngữ

1. Trong mục **Ngôn ngữ & Múi giờ**, tìm dòng **Ngôn ngữ**.
2. Nhấn vào danh sách thả xuống, chọn ngôn ngữ mong muốn:
   - **Tiếng Việt** (vi)
   - **English** (en)
   - **한국어** (ko)
3. Thay đổi có hiệu lực ngay; giao diện chuyển sang ngôn ngữ đã chọn mà không cần tải lại trang.

---

### Đổi múi giờ

1. Tìm dòng **Múi giờ** trong mục **Ngôn ngữ & Múi giờ**.
2. Nhấn vào bộ chọn, tìm kiếm hoặc cuộn đến múi giờ cần dùng (ví dụ: `Asia/Ho_Chi_Minh` cho GMT+7).
3. Chọn — lịch, ngày đến hạn và mốc thời gian trong ứng dụng tự điều chỉnh theo múi giờ mới.

---

### Đổi ngày đầu tuần

1. Tìm dòng **Ngày đầu tuần** _(First day of the week)_.
2. Chọn **Thứ Hai** hoặc **Chủ Nhật** tùy quy ước của bạn.
3. Tất cả lịch trong ứng dụng hiển thị lại theo lựa chọn này.

---

### Cấu hình thông báo email

1. Chọn tab **Thông báo** _(Notifications)_ trong menu bên trái.

{{screenshot:thong-bao-email-settings}}

2. Bật hoặc tắt từng loại thông báo bằng công tắc:

   | Loại thông báo           | Khi nào gửi email                                                         |
   | ------------------------ | ------------------------------------------------------------------------- |
   | **Thay đổi thuộc tính**  | Khi trạng thái, người gán, ưu tiên... của công việc bạn theo dõi thay đổi |
   | **Thay đổi trạng thái**  | Khi công việc chuyển sang trạng thái mới                                  |
   | — _Hoàn thành công việc_ | (cấp con) Khi công việc đạt trạng thái hoàn thành                         |
   | **Bình luận**            | Khi có bình luận mới trên công việc bạn theo dõi                          |
   | **Đề cập**               | Khi ai đó @mention bạn trong bình luận                                    |
   | **Nhắc nhở chấm công**   | Nhắc bạn ghi worklog nếu chưa ghi trong ngày                              |

3. Thay đổi lưu ngay lập tức — không cần nhấn nút Lưu.

## Mẹo & lưu ý

- **Múi giờ** ảnh hưởng đến cách hiển thị ngày/giờ trong toàn bộ ứng dụng. Nếu bạn thấy ngày đến hạn lệch, hãy kiểm tra múi giờ này trước.
- Thông báo email chỉ gửi cho các công việc bạn **đang theo dõi** (subscribed). Nếu không muốn nhận thông báo một công việc cụ thể, hủy theo dõi trực tiếp trên công việc đó.
- **Nhắc nhở chấm công** là tính năng riêng SHBVN — giúp đảm bảo worklog được ghi đầy đủ theo yêu cầu ngân hàng.
- Thay đổi giao diện tải lại trang; hãy lưu các thay đổi đang soạn trước khi đổi theme.

## Liên quan

- [Hồ sơ cá nhân](/help/a/ho-so-ca-nhan)
- [Inbox & thông báo](/help/a/inbox-va-thong-bao)
- [Chấm công & timesheet](/help/a/cham-cong-va-timesheet)
