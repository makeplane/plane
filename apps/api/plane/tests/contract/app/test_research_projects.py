...
class TestResearchProjectCreation:
    ...
    def test_second_active_project_is_rejected(self, env):
        assert (
            env["admin_client"]
            .post(env["url"], {"owner": str(env["member"].id)}, format="json")
            .status_code
            == 201
        )
        duplicate = env["admin_client"].post(
            env["url"], {"owner": str(env["member"].id)}, format="json"
        )
        assert duplicate.status_code == 409
        assert duplicate.json()["error_code"] == "research_project_exists"

    def test_multiple_projects_allowed_when_configured(self, env):
        ...
        second = env["admin_client"].post(env["url"], {"owner": str(env["member"].id)}, format="json")
    ...
    def test_member_cannot_create_for_someone_else(self, env):
        response = env["member_client"].post(
            env["url"], {"owner": str(env["admin"].id)}, format="json"
        )
...
class TestResearchProjectLifecycle:
    def _create(self, env, **payload):
        return env["admin_client"].post(
            env["url"], {"owner": str(env["member"].id), **payload}, format="json"
        ).json()
