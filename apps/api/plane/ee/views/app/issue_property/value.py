...
class IssuePropertyValueEndpoint(BaseAPIView):
    ...
    def get(self, request, slug, project_id, issue_id, property_id=None):
        ...
        if property_id:
            ...
            if issue_property_value:
                return Response(issue_property_value[0], status=status.HTTP_200_OK)
            return Response({"property_id": str(property_id), "values": []}, status=status.HTTP_200_OK)
    ...
    def post(self, request, slug, project_id, issue_id):
        try:
            ...
            formula_properties = IssueProperty.objects.filter(
                id__in=issue_property_ids, property_type=PropertyTypeEnum.FORMULA
            ...
            if formula_properties:
                for uuid, _ in formula_properties:
                    issue_property_values.pop(str(uuid))
            # ========== FORMULA PROPERTY ==========
    ...
    def patch(self, request, slug, project_id, issue_id, property_id):
        try:
            ...
            if issue_property.property_type == PropertyTypeEnum.FORMULA:
                return Response(
                    {"error": f"Formula property '{issue_property.display_name}' is read-only and cannot be edited"},
