...
def user_deactivation_email(current_site, user_id):
    try:
        ...
        context = with_email_branding({"email": str(user.email), "login_url": current_site + "/login"})
        subject = f"{user.first_name or user.display_name or user.email} has been deactivated on {context['brand_name']}"
