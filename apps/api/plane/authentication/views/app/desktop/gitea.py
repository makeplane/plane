...
class DesktopGiteaOauthInitiateEndpoint(View):
    def get(self, request):
        ...
        try:
            ...
            redirect_uri = f"{scheme}://{request.get_host()}/auth/desktop/gitea/callback/"

            state = uuid.uuid4().hex
            provider = GiteaOAuthProvider(request=request, state=state, redirect_uri=redirect_uri)
...
class DesktopGiteaCallbackEndpoint(View):
    def get(self, request):
