...
class DesktopGitLabOauthInitiateEndpoint(View):
    def get(self, request):
        ...
        try:
            ...
            redirect_uri = f"{scheme}://{request.get_host()}/auth/desktop/gitlab/callback/"

            state = uuid.uuid4().hex
            provider = GitLabOAuthProvider(request=request, state=state, redirect_uri=redirect_uri)
...
class DesktopGitLabCallbackEndpoint(View):
    def get(self, request):
