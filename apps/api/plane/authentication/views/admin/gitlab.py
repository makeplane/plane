...
class GitLabOauthInitiateAdminEndpoint(View):
...
class GitLabCallbackAdminEndpoint(View):
    def get(self, request):
        ...
        if state != request.session.get("state", ""):
            logger.warning(
                "State mismatch in GitLab admin callback",
                extra={
        ...
        if not code:
            logger.warning(
                "Code not found in GitLab admin callback",
                extra={
                    "error_code": AUTHENTICATION_ERROR_CODES["GITLAB_OAUTH_PROVIDER_ERROR"],
                    "error_message": "GITLAB_OAUTH_PROVIDER_ERROR",
                },
