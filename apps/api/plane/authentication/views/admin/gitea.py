...
class GiteaOauthInitiateAdminEndpoint(View):
...
class GiteaCallbackAdminEndpoint(View):
    def get(self, request):
        ...
        if state != request.session.get("state", ""):
            logger.warning(
                "State mismatch in Gitea admin callback",
                extra={
        ...
        if not code:
            logger.warning(
                "Code not found in Gitea admin callback",
                extra={
                    "error_code": AUTHENTICATION_ERROR_CODES["GITEA_OAUTH_PROVIDER_ERROR"],
                    "error_message": "GITEA_OAUTH_PROVIDER_ERROR",
                },
