...
class GitHubOauthInitiateAdminEndpoint(View):
...
class GitHubCallbackAdminEndpoint(View):
    def get(self, request):
        ...
        if state != request.session.get("state", ""):
            logger.warning(
                "State mismatch in GitHub admin callback",
                extra={
        ...
        if not code:
            logger.warning(
                "Code not found in GitHub admin callback",
                extra={
                    "error_code": AUTHENTICATION_ERROR_CODES["GITHUB_OAUTH_PROVIDER_ERROR"],
                    "error_message": "GITHUB_OAUTH_PROVIDER_ERROR",
                },
