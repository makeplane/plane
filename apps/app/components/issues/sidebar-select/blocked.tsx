    ...
    <div className="flex flex-wrap items-start py-2">
      ...
      <div className="space-y-1 sm:basis-1/2">
        <div className="flex flex-wrap gap-1">
          {watch("blocked_list") && watch("blocked_list").length > 0
            ? watch("blocked_list").map((issue) => (
                <div
                  key={issue}
                  className="group flex cursor-pointer items-center gap-1 rounded-2xl border border-skin-base px-1.5 py-0.5 text-xs text-red-500 duration-300 hover:border-red-500 hover:bg-red-500/30"
                >
                  <Link
        ...
        >
          <Dialog as="div" className="relative z-20" onClose={handleClose}>
            ...
            <div className="fixed inset-0 z-20 overflow-y-auto p-4 sm:p-6 md:p-20">
              ...
              >
                <Dialog.Panel className="relative mx-auto max-w-2xl transform divide-y divide-gray-500 divide-opacity-10 rounded-xl bg-skin-surface-2 bg-opacity-80 shadow-2xl ring-1 ring-black ring-opacity-5 backdrop-blur backdrop-filter transition-all">
                  <form>
                    <Combobox
                      onChange={(val: string) => {
                    ...
                    >
                      <div className="relative m-1">
                        <MagnifyingGlassIcon
                          className="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-skin-base text-opacity-40"
                          aria-hidden="true"
                        />
                        <input
                          type="text"
                          className="h-12 w-full border-0 bg-transparent pl-11 pr-4 text-skin-base placeholder-gray-500 outline-none focus:ring-0 sm:text-sm"
                          placeholder="Search..."
                      ...
                      >
                        {filteredIssues.length > 0 ? (
                          <li className="p-2">
                            {query === "" && (
                              <h2 className="mt-4 mb-2 px-3 text-xs font-semibold text-skin-base">
                                    ...
                                    <Combobox.Option
                                      ...
                                      className={({ active }) =>
                                        `flex cursor-pointer select-none items-center justify-between rounded-md px-3 py-2 ${
                                          active ? "bg-gray-900 bg-opacity-5 text-skin-base" : ""
                                        }`
                                      }
                                    >
                                      <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="flex-shrink-0 text-xs text-skin-muted-2">
                                          {
                          ...
                          <div className="flex flex-col items-center justify-center gap-4 px-3 py-8 text-center">
                            ...
                            <h3 className="text-skin-muted-2">
                              No issues found. Create a new issue with{" "}
                              <pre className="inline rounded bg-skin-surface-2 px-2 py-1">C</pre>.
                            </h3>
                          </div>
                        )}
                      </Combobox.Options>
        ...
        <button
          ...
          } items-center justify-between gap-1 rounded-md border border-skin-base px-2 py-1 text-xs shadow-sm duration-300 focus:border-skin-accent focus:outline-none focus:ring-1 focus:ring-skin-accent`}
