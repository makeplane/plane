...
export const CompletedIssuesGraph: React.FC<Props> = ({ month, issues, setMonth }) => {
  ...
  return (
    <div>
      ...
      <div className="rounded-[10px] border border-custom-border-80 bg-custom-background-100 p-8 pl-4">
        {data.every((item) => item.completed_count === 0) ? (
          ...
          <>
            <LineGraph
              ...
              enableSlices="x"
              sliceTooltip={(datum) => (
                <div className="rounded-md border border-custom-border-80 bg-custom-background-80 p-2 text-xs">
