...
const ContextMenu = ({ position, children, title, isOpen, setIsOpen }: Props) => {
  ...
  return (
    ...
    >
      <div
        className={`fixed z-20 flex min-w-[8rem] flex-col items-stretch gap-1 rounded-md border border-custom-border-80 bg-custom-background-90 p-2 text-xs shadow-lg`}
        style={{
          left: `${position.x}px`,
          top: `${position.y}px`,
        }}
      >
        {title && (
          <h4 className="border-b border-custom-border-80 px-1 py-1 pb-2 text-[0.8rem] font-medium">
            {title}
          </h4>
        )}
        {children}
