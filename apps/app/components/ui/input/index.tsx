...
export const Input: React.FC<Props> = ({
  ...
  <>
    {label && (
      <label htmlFor={id} className="mb-2 text-skin-muted-1">
        {label}
      </label>
    )}
    <input
      ...
      className={`block rounded-md bg-transparent text-sm focus:outline-none ${
        mode === "primary"
          ? "rounded-md border border-skin-base"
          : mode === "transparent"
          ? "rounded border-none bg-transparent ring-0 transition-all focus:ring-1 focus:ring-skin-accent"
