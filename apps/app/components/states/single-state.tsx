...
export const SingleState: React.FC<Props> = ({
  ...
  return (
    <div
      className={`group flex items-center justify-between gap-2 border-skin-base bg-skin-surface-2 p-5 first:rounded-t-[10px] last:rounded-b-[10px]`}
    >
      <div className="flex items-center gap-3">
        {getStateGroupIcon(state.group, "20", "20", state.color)}
        <div>
          <h6 className="font-medium text-skin-muted-1">{addSpaceIfCamelCase(state.name)}</h6>
      ...
      <div className="flex items-center gap-2">
        {index !== 0 && (
          <button
            type="button"
            className="hidden text-gray-400 hover:text-skin-base group-hover:inline-block"
        ...
        {state.default ? (
          ...
          <button
            type="button"
            className="hidden text-xs text-gray-400 hover:text-skin-base group-hover:inline-block"
