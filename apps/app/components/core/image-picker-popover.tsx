...
export const ImagePickerPopover: React.FC<Props> = ({ label, value, onChange }) => {
  ...
  return (
    <Popover className="relative z-[2]" ref={ref}>
      <Popover.Button
        className="rounded-md border border-skin-base bg-skin-surface-2 px-2 py-1 text-xs text-gray-700"
      ...
      >
        <Popover.Panel className="absolute right-0 z-10 mt-2 rounded-md bg-skin-surface-2 shadow-lg">
          <div className="h-96 w-80 overflow-auto rounded border border-skin-base bg-skin-surface-2 p-5 shadow-2xl sm:max-w-2xl md:w-96 lg:w-[40rem]">
            <Tab.Group>
              <Tab.List as="span" className="inline-block rounded bg-skin-surface-2 p-1">
                {tabOptions.map((tab) => (
                  <Tab
                    key={tab.key}
                    className={({ selected }) =>
                      `rounded py-1 px-4 text-center text-sm outline-none transition-colors ${
                        selected ? "bg-skin-accent text-white" : "text-skin-base"
                      }`
                    }
                  >
                    {tab.title}
