    ...
    <StrictModeDroppable droppableId={date.date}>
      {(provided) => (
        <div
          ...
          className={`group relative flex min-h-[150px] flex-col gap-1.5 border-t border-custom-border-80 p-2.5 text-left text-sm font-medium hover:bg-custom-background-90 ${
            isMonthlyView ? "" : "pt-9"
          } ${
            showWeekEnds
              ? (index + 1) % 7 === 0
        ...
        >
          ...
          {totalIssues > 4 && (
            <button
              type="button"
              className="w-min whitespace-nowrap rounded-md border border-custom-border-80 bg-custom-background-80 px-1.5 py-1 text-xs"
