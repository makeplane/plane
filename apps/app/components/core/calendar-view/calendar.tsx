      ...
      <div className="h-full overflow-y-auto rounded-lg text-gray-600 -m-2">
        <div className="mb-4 flex items-center justify-between">
                ...
                <>
                  <Popover.Button className={`group flex h-full items-start gap-1 text-gray-800`}>
                    <div className="flex  items-center   justify-center gap-2 text-2xl font-semibold">
                      <span className="text-skin-base">{formatDate(currentDate, "Month")}</span>{" "}
                  ...
                  >
                    <Popover.Panel className="absolute top-10 left-0 z-20 w-full max-w-xs flex flex-col transform overflow-hidden bg-skin-surface-2 shadow-lg rounded-[10px]">
                      ...
                      <div className="grid grid-cols-4  px-2 border-t border-skin-base">
          ...
          <div className="flex w-full gap-2 items-center justify-end">
            <button
              className="group flex cursor-pointer items-center gap-2 rounded-md border border-skin-base bg-skin-surface-2 px-4 py-1.5 text-sm  hover:bg-skin-surface-1 hover:text-skin-base focus:outline-none"
              onClick={() => {
                if (isMonthlyView) {
                  updateDate(new Date());
                } else {
            ...
            <CustomMenu
              customButton={
                <div
                  className={`group flex cursor-pointer items-center gap-2 rounded-md border border-skin-base bg-skin-surface-2 px-3 py-1.5 text-sm  hover:bg-skin-surface-1 hover:text-skin-base focus:outline-none `}
                >
            ...
            >
              ...
              <div className="mt-1 flex w-52 items-center justify-between border-t border-skin-base py-2 px-1  text-sm text-gray-600">
                <h4>Show weekends</h4>
                <button
                  type="button"
                  className={`relative inline-flex h-3.5 w-6 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    showWeekEnds ? "bg-green-500" : "bg-skin-surface-2"
                  }`}
                  role="switch"
        ...
        >
          {weeks.map((date, index) => (
            <div
              key={index}
              className={`flex  items-center justify-start p-1.5 gap-2 border-skin-base bg-skin-surface-1 text-base font-medium text-gray-600 ${
                !isMonthlyView
                  ? showWeekEnds
                    ? (index + 1) % 7 === 0
                      ? ""
            ...
            <StrictModeDroppable droppableId={date.date}>
              {(provided, snapshot) => (
                <div
                  ...
                  className={`flex flex-col gap-1.5 border-t border-skin-base p-2.5 text-left text-sm font-medium hover:bg-skin-surface-1 ${
                    showWeekEnds
                      ? (index + 1) % 7 === 0
                        ? ""
                        : "border-r"
                      ...
                      <Draggable draggableId={issue.id} index={index}>
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`w-full cursor-pointer truncate rounded bg-skin-surface-2 p-1.5 hover:scale-105 ${
