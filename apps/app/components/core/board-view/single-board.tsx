...
export const SingleBoard: React.FC<Props> = ({
  ...
  return (
    <div className={`h-full flex-shrink-0 ${!isCollapsed ? "" : "w-96 bg-skin-base"}`}>
            ...
            {(provided, snapshot) => (
              <div
                className={`relative h-full overflow-y-auto p-1  ${
                  snapshot.isDraggingOver ? "bg-skin-accent bg-opacity-50" : ""
              ...
              >
                {orderBy !== "sort_order" && (
                  <>
                    <div
                      ...
                      } pointer-events-none top-0 left-0 z-[99] h-full w-full bg-skin-surface-1 opacity-50`}
                    />
                    <div
                      className={`absolute ${
                        snapshot.isDraggingOver ? "block" : "hidden"
                      } pointer-events-none top-1/2 left-1/2 z-[99] -translate-y-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-skin-surface-2 p-2 text-xs`}
                    >
                ...
                {type === "issue" ? (
                  <button
                    type="button"
                    className="flex items-center gap-2 font-medium text-skin-accent outline-none"
