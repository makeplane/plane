...
export const SingleBoardIssue: React.FC<Props> = ({
  ...
  return (
    <>
      ...
      <div
        className={`mb-3 rounded bg-skin-surface-2 shadow ${
          snapshot.isDragging ? "border-2 border-skin-accent shadow-lg" : ""
        }`}
      ...
      >
        <div className="group/card relative select-none p-3.5">
          ...
          <Link href={`/${workspaceSlug}/projects/${issue.project}/issues/${issue.id}`}>
            <a>
              {properties.key && (
                <div className="mb-2.5 text-xs font-medium text-skin-muted-2">
                  {issue.project_detail.identifier}-{issue.sequence_id}
                </div>
              )}
              <h5
                className="break-all text-sm group-hover:text-skin-accent"
          ...
          <div className="relative mt-2.5 flex flex-wrap items-center gap-2 text-xs">
            ...
            {properties.sub_issue_count && (
              <div className="flex flex-shrink-0 items-center gap-1 rounded-md border border-skin-base px-3 py-1.5 text-xs shadow-sm">
