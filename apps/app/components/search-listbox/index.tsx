    ...
    <Combobox as="div" {...props} className="relative flex-shrink-0">
      {({ open }: any) => (
        <>
          ...
          <Combobox.Button
            className={`flex cursor-pointer items-center gap-1 rounded-md border  border-skin-base px-2 py-1 text-xs shadow-sm duration-300 hover:bg-skin-surface-1 focus:border-skin-accent focus:outline-none focus:ring-1 focus:ring-skin-accent ${
              buttonClassName || ""
            }`}
          >
            {icon ?? null}
            <span
              className={`hidden truncate sm:block ${
                value === null || value === undefined ? "" : "text-skin-base"
              }`}
            >
              {Array.isArray(value)
                ? value
          ...
          >
            <Combobox.Options
              className={`absolute z-10 mt-1 max-h-32 min-w-[8rem] overflow-auto rounded-md bg-skin-surface-2 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none  ${
                optionsFontsize === "sm"
                  ? "text-xs"
                  : optionsFontsize === "md"
                  ? "text-base"
              ...
              <div className="py-1">
                {filteredOptions ? (
                  filteredOptions.length > 0 ? (
                    ...
                    <p className="text-xs text-skin-muted-2 px-2">No {title.toLowerCase()} found</p>
                  )
                ) : (
                  <p className="text-xs text-skin-muted-2 px-2">Loading...</p>
                )}
              </div>
            </Combobox.Options>
          </Transition>
