...
export const SingleLabelGroup: React.FC<Props> = ({
  ...
  return (
    <Disclosure
      as="div"
      className="rounded-[10px] border border-custom-border-80 bg-custom-background-100 p-5 text-custom-text-100"
      defaultOpen
    >
      {({ open }) => (
        <>
            ...
            <Disclosure.Panel>
              <div className="mt-3 ml-6 space-y-3">
                {labelChildren.map((child) => (
                  <div
                    key={child.id}
                    className="group flex items-center justify-between rounded-md border border-custom-border-80 p-2 text-sm"
                  >
