...
const EmojiIconPicker: React.FC<Props> = ({
  ...
  return (
    <Popover className="relative z-[1]" ref={ref}>
      <Popover.Button
        className="rounded-full bg-skin-surface-1 p-2 outline-none sm:text-sm"
      ...
      >
        <Popover.Panel className="absolute z-10 mt-2 w-[250px] rounded-[4px] bg-skin-surface-2 shadow-lg">
          <div className="h-[230px] w-[250px] overflow-auto border border-skin-base rounded-[4px] bg-skin-surface-2 p-2 shadow-xl">
                    ...
                    <div className="py-2">
                      ...
                      <div className="grid grid-cols-10">
                        {recentEmojis.map((emoji) => (
                          <button
                            type="button"
                            className="h-4 w-4 select-none text-sm hover:bg-skin-surface-2 flex items-center justify-between"
                            key={emoji}
                            onClick={() => {
                              onChange(emoji);
                              setIsOpen(false);
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-10 gap-y-1">
                      {emojis.map((emoji) => (
                        <button
                          type="button"
                          className="h-4 w-4 mb-1 select-none text-sm hover:bg-skin-surface-2 flex items-center"
                          key={emoji}
                          onClick={() => {
                            onChange(emoji);
                  ...
                  <Tab.Panel className="flex h-full w-full flex-col justify-center">
                    <div className="grid grid-cols-10 mt-1 ml-1 gap-1">
                      {icons.material_rounded.map((icon) => (
                        <button
                          type="button"
                          className="h-4 w-4 mb-1 select-none text-lg hover:bg-skin-surface-2 flex items-center"
                          key={icon.name}
                          onClick={() => {
