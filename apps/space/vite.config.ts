...
// Django builds its redirects from the configured WEB_URL/SPACE_BASE_URL, so a
...
const redirectBaseUrls = [process.env.VITE_WEB_BASE_URL, process.env.VITE_SPACE_BASE_URL]
  .map((url) => (url || "").replace(/\/+$/, ""))
  .filter(Boolean);
