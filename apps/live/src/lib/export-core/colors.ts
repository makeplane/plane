 ...
 * to ensure exports match the editor's appearance.
 *
 * Source mappings:
...
// Use light theme colors by default for exports
...
/**
 * Resolves a color value to a hex color for rendering.
...
export const resolveColor = resolveColorForPdf;

/**
 * Strips the '#' prefix from a hex color for DOCX compatibility.
 * DOCX requires hex colors without the '#' prefix.
 */
export const hexToDocxColor = (hex: string): string => {
  if (hex.startsWith("#")) {
    return hex.slice(1);
  }
  return hex;
...
/**
 * Resolves a color value and strips '#' for DOCX format.
 * Combines resolveColorForPdf with hex stripping.
 */
export const resolveColorForDocx = (value: string | null | undefined, type: "text" | "background"): string | null => {
  const resolved = resolveColorForPdf(value, type);
  if (!resolved) return null;
  return hexToDocxColor(resolved);
};
