...
| 5. Drag Handler Integration & Error UX                       | 0/2            | Planned     | -          |
...
- [ ] 05-01-PLAN.md — Wave 1 typed seam: 10 i18n keys under `timeline.propagation.*` in en + ja `translations.ts` (D-06 / D-06a / D-06b — Ubiquitous Language honored); NEW `apps/web/core/hooks/store/use-timeline-propagation-store.ts` (mirrors `use-instance.ts` analog); NEW `apps/web/core/components/gantt-chart/helpers/propagation/toast-resolver.ts` exposing `MESSAGE_KEY_BY_CODE` (Record over the 7 wire codes — exhaustiveness check), `showPropagationErrorToast(code | "UNEXPECTED", t)` (D-04 / D-04c — single ERROR severity, shared title key, no action buttons), and `showHiddenUpdateToast(count, t)` (D-05 / D-05a — INFO toast with ICU plural). Pure-function module; no React hook. Type-only imports per D-12. Addresses ERR-01..ERR-07.
- [ ] 05-02-PLAN.md — Wave 2 drag-handler wiring: UPDATE `use-gantt-resizable.ts` adding optional 5th `propagationCallbacks` param (D-03b gate — null = skip; module/cycle/project Gantt unchanged) wiring `beginPreview` at mousedown with `block.data.updated_at` snapshot (D-09 — Pitfall 5) and `updatePreview` per mousemove (D-02 — implicit throttle via day-quantization); resize / left / right branches BYTE-IDENTICAL (FE-09). UPDATE `base-gantt-root.tsx::updateBlockDates` implementing D-01 split predicate (single entry, both dates, dragged-id, pre-drag had both dates), routing move → `commitWithServerResult` (D-01) and resize/half-block → `issues.updateIssueDates` (D-01b verbatim); on success fires `showHiddenUpdateToast(hiddenUpdateCount, t)` when > 0 (D-05 / D-05b / D-05c); on failure fires `showPropagationErrorToast(unexpectedError ? "UNEXPECTED" : result.code, t)` (D-04 / D-04c). Edges assembler iterates `relation.relationMap[srcId]?.blocking` only (D-03a — Pitfall 2 single-direction). UPDATE `block.tsx` overriding `marginLeft` / `width` from `previewById.get(blockId)` via `getPositionFromDateOnGantt` when present, else `block.position` fallback (D-02 / D-02b — observer reactivity; D-02a dragged block keeps direct DOM writes). 4 CE dependency-drag files + Phase 4 store + `issue.service.ts` byte-identical (FE-08 / FE-09 / D-10b / Phase 4 D-03b inert). Manual smoke checklist (14 scenarios, D-11a) gates /gsd-verify-work. Addresses FE-03, FE-09, ERR-08.
