...
| 3. Propagation API Endpoint, Persistence & Contract          | 0/3            | Planned     | -          |
...
- [ ] 03-01-PLAN.md — Wave 1 routing scaffold: extend `tests/factories.py` with `IssueFactory` / `IssueRelationFactory` / `StateFactory`; create empty `TimelinePropagationView(BaseAPIView)` returning a 501 placeholder; create placeholder serializer module; register URL `project-timeline-propagation`; barrel re-exports; add 6 GREEN contract tests (3 factory sanity + `test_url_reverses` + `test_unauthenticated_request_returns_401` + `test_existing_bulk_update_endpoint_unchanged`) — addresses API-01, API-09 (partial), API-11, API-12 (test scaffold), TEST-18 (partial).
- [ ] 03-02-PLAN.md — Wave 2 serializers + view body + happy/failure paths: implement `TimelinePropagationRequestSerializer` / `Response` / `Error` / `WorkItem` (D-04 structural-only), full `TimelinePropagationView.post` body with inline `ProjectMember` permission check (D-02), `transaction.atomic()` + `select_for_update(of=("self",))` (D-05), queryset construction with cross-project annotations (D-10, D-11), `propagate_move`, `Issue.objects.bulk_update(..., ["start_date","target_date","updated_at"])` with single captured `now` (D-05a / D-05f), and the `{code, message}` failure envelope per D-03 HTTP status mapping. NO `transaction.on_commit` yet (Plan 03-03 owns that). Adds 16 contract tests (5 serializer + 11 view = chain success, single-now, 5 domain failure envelopes, stale 409, non-member / GUEST / not-in-project 403). Addresses API-02, API-03, API-04, API-05, API-06, API-07, API-08, API-10, PROP-16, TEST-10, TEST-13, TEST-15, TEST-16, TEST-17.
- [ ] 03-03-PLAN.md — Wave 3 audit + webhook fan-out via `transaction.on_commit`: append per-pair `issue_activity.delay(...)` (start_date / target_date events) + per-issue `model_activity.delay(...)` registrations to the success path with default-arg lambda capture (RESEARCH Pitfall 4); first usage of `transaction.on_commit` in `apps/api/plane`; 3 new contract tests pinning the on_commit pattern, the cycle-failure-no-fanout regression (Pitfall 7), and the django_db transaction-never-commits trap (Pitfall 9). Addresses API-12.
