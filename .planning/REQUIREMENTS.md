...
- [x] **API-01**: 専用の propagation endpoint が同一プロジェクト範囲で利用できる(Implementation Decision: dedicated endpoint) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-02**: client は move intent(`work_item_id`, 元の schedule、`requested_start_date`, `requested_target_date`, `operation: "move"`)を送る。precomputed update list は受け付けない(US-35, Implementation Decision) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-03**: 成功レスポンスは更新された全 Work Item の `id` / `start_date` / `target_date` / `updated_at` を返す(US-36) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-04**: 成功レスポンスは propagation メタ情報(`requested_work_item_id`, `total_updated_count`, optional `client_preview_count`)を含む(Implementation Decision) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-05**: 失敗レスポンスは安定した `{ code, message }` オブジェクトを返す(US-22, US-37) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-06**: 初期失敗 code は `DEPENDENCY_CYCLE` / `PROJECT_BOUNDARY_EXCEEDED` / `INCOMPLETE_SCHEDULE` / `PROPAGATION_LIMIT_EXCEEDED` / `SCHEDULE_CHANGED` / `PERMISSION_DENIED` / `INVALID_DATE_RANGE` の 7 種(Implementation Decision) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-07**: client が drag 開始時に持っていた `updated_at` または version と現在のサーバ値が乖離する場合は `SCHEDULE_CHANGED` で fail(US-27) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-08**: 失敗時は **Work Item の dates が一切更新されない**(all-or-nothing 永続化)(US-21, Testing Decision) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-09**: 既存 project permission を再利用し、unauthorized は `PERMISSION_DENIED` で fail(US-31) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-10**: 不正な date range(例: `target_date < start_date`)は `INVALID_DATE_RANGE` で fail — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [x] **API-11**: 既存 bulk date update endpoint は触らず、非伝播経路として残す(Implementation Decision) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
...
- [x] **TEST-10**: backend service unit test: cross-project dependency path → `PROJECT_BOUNDARY_EXCEEDED` — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
...
- [x] **TEST-13**: backend service unit test: stale schedule rejection → `SCHEDULE_CHANGED` — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
- [ ] **TEST-14**: backend service unit test: invalid date range → `INVALID_DATE_RANGE`
- [x] **TEST-15**: API contract test: 任意の失敗で permanent persistence が起きない(all-or-nothing) — Phase 3 (2026-05-04): completed by `apps/api/plane/app/views/issue/timeline_propagation.py` + `apps/api/plane/app/serializers/timeline_propagation.py` + `apps/api/plane/tests/contract/app/test_timeline_propagation.py` (26 GREEN contract tests). See `.planning/phases/03-propagation-api-endpoint-persistence-contract/03-VERIFICATION.md`.
...
| FE-05       | Phase 4 | Done (Plan 04-02, 2026-05-04 — TimelinePropagationStore failure path discards previewById via \_isProtocolError + rollback; canonical issues map untouched on failure)  |
...
| TEST-20     | Phase 4 | Done (Plan 04-02, 2026-05-04 — covered transitively via D-04c helper immutability invariant + TimelinePropagationStore.rollback() one-line clear + Phase 6 E2E TEST-24) |
