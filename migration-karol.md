...
- **Change type on existing items** — the detail/peek header badge (`IssueTypeSwitcher`) is now a picker: click the badge → choose a type → PATCHes `type_id` (display-only when not editable). **Execution fix (2026-09-08): the main API's `IssueCreateSerializer` silently dropped `type_id`** (the model FK is named `type`) — added the explicit `type_id` field; the switcher now persists. **Second fix (2026-09-09, Phase 4 review — Karol found the UI still didn't change the type): the combobox dropdown itself never fired** — the headlessui `Combobox.Button as={Fragment}` onClick composition (custom button + `preventDefault` in `useDropdown`) swallowed the option click, so selecting a type produced no request at all (no PATCH, no activity, DB unchanged — while the API PATCH worked). `dropdowns/type.tsx` was **rewritten without headlessui** (plain React state + buttons: own outside-click/Escape handling, popper-free absolute menu) — the modal selector and the badge picker share it. Follow-up tickets: **project-level issue types with workspace fallback** (QUESTIMUS-29) and **sort by Name/ID asc+desc** (QUESTIMUS-28).
