...
| **2. Access & views** | Bot users + tokens (§6.2 — incl. personal-project bots; tokens saved to a gitignored local file for Phase 8/9 wiring); views via `setup-workspace.js` (`config/setup.json` `views` section — workspace "Next" + per-project In Progress/Done/User reports/Blocked/Planning/Tickets + Personal Inbox/Ideas/Someday; needs the §7.5 fork change from the Phase 1 rebuild); Questimus project content via `create-questimus-content.js` (this plan → page + the 8 tickets) | Tokens work, views in place, Questimus content live |
...
2. **Views:** re-run `node setup-workspace.js --config config/setup.json` (projects/states/labels already exist → only the views are created: workspace "Next" + per-project In Progress/Done/User reports/Blocked/Planning/Tickets + Personal Inbox/Ideas/Someday; the type views need the §7.5 fork change from the Phase 1 rebuild).
