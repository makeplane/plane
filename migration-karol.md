...
- **Phase 3 (2026-09-08): implemented.** The workspace home (`/{workspaceSlug}`) shows three fixed sections below the widgets — **My Issues** (assigned to me across projects; Pending/Upcoming/Overdue/Marked-completed tabs), **"Now"** (open + assigned + urgent/high **or** due ≤ 7 days) and **"Today"** (open + assigned + due today/overdue) — computed per request from the workspace issues endpoint (`assignee_id__in` rich filter). Labels are hardcoded EN for now (i18n keys deferred). Note: the workspace home was already routed (`WorkspaceHomeView` at the workspace index — the "dead code" claim was stale); what was missing was the My Issues/Now/Today content.
