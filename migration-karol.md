...
| Source                                                  | Volume                                                                                                                                                                                                                  | Notes                                                                                                                           |
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `Projects\Legaliosa-test-ox-alpha\planning`             | **220 tickets** (159 open + 61 done), 11 SP files, 38 T units, plan.md, design/ (79 files: 23 HTML mockups + 43 PNG + 5 md — **moved to repo root `design/` by Karol 2026-09-09**, not migrated), archive/v1, v2-replan-input                                                                | **Active Legaliosa (v2)** — migrated §10 (Phase 8, 2026-09-09)                                                                                 |
| `Projects\Legaliosa\planning`                           | **101 tickets**, SP01–10, mockups/                                                                                                                                                                                      | Legaliosa **v1** — superseded by v2                                                                                             |
| `Projects\Jobernaut2\planning`                          | **77 tickets** (73 open + 4 done), 7 active SP files (SP-13..19), 22 T units, PLAN.md, HANDOFF.md, PHASE-08.md, pilot-findings.md, design/; `archive\v1\subplans\` holds 12 v1 subplans (**imported as "Plan v1" hierarchy — Karol 2026-09-09**) | **Active Jobernaut (v2)** — j2mainlink removed by Karol 2026-09-08 (O3); **migrated §10 (Phase 9, 2026-09-09)** |
...
**Total migration volume (dry-run verified, 0 failures; live sources drift — Jobernaut +2 on 2026-09-08): 559 issues + 32 pages + 120 relations** (505 project issues + 54 Now-routing). Small enough to migrate everything (including done tickets) with scripts. **Revised 2026-09-09 (O13): 551 issues + 30 pages remain** — Karol deleted the pm folders for Legaliosa, Media Consumerus, Kleinanzeigen and POP (content re-created manually as tickets; §9.2). **Revised again 2026-09-09 (Phase 5 review): 562 issues + 24 pages** — the 3 Media Consumerus pages became doc tickets, 2 doc tickets were deleted after their content was distributed into the standalone issues, 3 deduped TODOS sections got their own deferred tickets, and the 7 archive/v1 plans were imported as done tickets (§5.5, §9.2). **Revised 2026-09-09 (Phase 6): 613 issues + 24 pages** — Arsenal + Empirium vault planning imported (47 + 3 plans, 26 pm tickets, HANDOFF page; §9.2). **Revised 2026-09-09 (Phase 7): 647 issues + 24 pages** — Don Saldo imported (4 + 30; §9.2). **Revised 2026-09-09 (Phase 7 follow-up): 660 issues + 24 pages** — Don Saldo `docs/plans` imported (13 Plan issues, In Progress). **Revised 2026-09-09 (Phase 8): 680 issues + 19 pages** — Legaliosa imported (281 issues + 86 relations + 6 pages; v1 archive → "Plan v1" hierarchy; design reviews skipped). **Revised 2026-09-09 (Phase 9): 701 issues + 16 pages** — Jobernaut imported (120 issues + 34 relations + 7 pages; v1 archive → "Plan v1" hierarchy; design skipped).
...
| research notes                     | **Ticket bodies** (Karol 2026-09-08, supersedes the pages approach): `own-server-vs-vps-etc.md` → **PERSONAL-12**, `how-does-claudian…flatrate-subscription.md` → **PERSONAL-29**; all 4 pages deleted. General rule for the remaining phases: a standalone doc with a sibling issue → merge the content into the issue description; only docs **without** a sibling become pages. **Revised 2026-09-09 (Phase 5 review):** the Media Consumerus pages (Backlog / TODOS / Design — DESIGN) were **transformed into tickets** (full document content in the description, pages archived + deleted). The Backlog and TODOS doc tickets were then **deleted** after their content was distributed: each E-issue got its detailed `### E<n>` section, TODOS-E2 got its "original note", and the 3 deduped TODOS sections became their own deferred tickets. **Design — DESIGN stays as a ticket** (no sibling issues). Plain documents live in ticket bodies; pages remain only for design reviews/decisions (O8) and **living documents** — the Arsenal **HANDOFF** is a page, not a work item (Karol 2026-09-09: "a doc that is alive forever" — constantly updated/trimmed/appended). The `archive/v1` plans were imported as **done Plan tickets** (2026-09-09) |
...
| Pages   | `page_view`        | **on** (all projects) | the plan creates 26 pages (design reviews O8, POP doc, Questimus plan page, …; Personal research/hosting docs merged into ticket bodies — Karol 2026-09-08, §5.5; Media Consumerus docs → tickets 2026-09-09, §5.5) |
...
**Execution finding (Phase 7, 2026-09-09): bots cannot create labels.** `ProjectMemberPermission` gates `POST /labels/` on a **workspace** Admin/Member role (`plane/app/permissions/project.py` — the workspace Guest role the bots use is not enough), so any import that needs a label the project doesn't have yet 403s. The importer's `ensureLabel` only creates missing labels — the labels must **pre-exist** (created by `setup-workspace.js` with Karol's token, by `import-now.js` routing, or DB-side). Pre-created: `todo` in Don Saldo, `idea` in Jobernaut, **SP-01..SP-05 + SP-07..SP-10 in Legaliosa** (Phase 8 — the ticket `sub_plan` values are normalized to SP-XX labels; non-SP values `unknown`/`post-v1`/`pre-billing`/`v1.5`/`none` → **no label**, per Karol 2026-09-09).
...
**Questimus first as the pilot** (decided 2026-09-09 — it's a working app; mockup → code → template, tracked as QUESTIMUS-39), then Don Saldo, Jobernaut, Legaliosa, Media Consumerus. Each app gets a small shared module (report-issue form + API client) so the pattern is identical everywhere; per-project design adjustments and field differences are decided when each app is reached.
...
| ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------- | --------- |
| legaliosa-v2                                                                                    | 281 (2 Plan + 21 SP + 38 T + 220 tickets; v1 archive imported as "Plan v1" + 10 SP — Karol 2026-09-09, §9.3) | 6               | 86        |
| jobernaut-v2                                                                                    | 120 (2 Plan + 19 SP + 22 T + 77 tickets; v1 archive imported as "Plan v1" + 12 SP — Karol 2026-09-09, §9.3) | 7               | 34        |
| don-saldo                                                                                       | 4 (2 tickets + bank-import note + brand playbook doc tickets)                    | 0 (pages → doc tickets 2026-09-09) | 0         |
| media-consumerus                                                                                | 23 (10 backlog + 2 todos + 3 deferred + 7 archive plans + Design doc ticket; doc tickets Backlog/TODOS deleted after content distribution; +4 deduped section ids) | 0 (pages → tickets 2026-09-09) | 0         |
| personal (Ideas only — Now.md moved to now-routing)                                             | 23                                                                                | 0               | 0         |
| jobernaut-pm                                                                                    | 48                                                                                | 0               | 0         |
| don-saldo-pm                                                                                    | 30 (29 checkboxes + Change-to-use-Docker doc ticket)                            | 0               | 0         |
| don-saldo-plans (docs/plans — Phase 7 follow-up)                                                 | 13 (all type Plan, state In Progress — Karol 2026-09-09)                        | 0               | 0         |
...
| arsenal-pm (Arsenal.md task list)                                                               | 24 (23 checkboxes + improve-subagent-config doc ticket)                           | 0 (page → doc ticket 2026-09-09) | 0         |
| arsenal-planning (vault planning — Phase 6)                                                     | 47 (10 active + 14 done + 3 plan-10-11 + 3 archive + 17 initial-repo-triage; superseded-skill subfolders excluded) | 1 (HANDOFF page) | 0         |
| empirium-planning (vault planning — Phase 6)                                                    | 3                                                                                 | 0               | 0         |
...
| **Total**                                                                                       | **701** (559 − 8 + 3 doc − 2 deleted + 3 deferred + 7 archive + 1 pm doc + 47 + 3 + 4 + 30 + 13 + 20 + 21) | **16** (29 − 2 − 3 − 1 + 1 − 5 − 3) | **120**   |
...
| Source                                                          | Action                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Legaliosa-test-ox-alpha\planning`                              | 220 tickets → issues; 11 SP → Plan issues; 38 T → Plan issues; plan.md → Plan issue (**"Plan v2"** — Karol); **archive/v1 → imported as Done** (plan.md → **"Plan v1"** + SP01–SP10 nested under it — Karol 2026-09-09, reverses the earlier exclusion); HANDOFF/pilot-findings/v2-replan-input → pages; **design reviews skipped** (design folder stays in repo — Karol 2026-09-09); archive stays in repo |
| `Jobernaut2\planning`                                           | Same pattern: 77 tickets, 7 SP (SP-13..19), 22 T, PLAN.md → Plan issue (**"Plan v2"** — Karol); **archive/v1 → imported as Done** (PLAN.md → **"Plan v1"** + subplans SP-01..12 folded under it; _index.md skipped; v2-replan-input → pages — Karol 2026-09-09); design/ skipped (stays in repo) — **imported 2026-09-09 (Phase 9)**                                                                                                                                                                                                   |
| `Don-Saldo\planning`                                            | 2 tickets → issues; bank-import note → page — **imported 2026-09-09 (Phase 7)**: bank-import note + brand playbook → **doc tickets** (pages → tickets, §5.5)                                                                                                                                                                                                                                                                                             |
| `Don-Saldo\docs\plans`                                          | **13 plan files → type Plan, state In Progress** (Karol 2026-09-09, Phase 7 follow-up; incl. the 332 KB `plan-v1.md`) — **imported 2026-09-09**                                                                                                                                                                                                                                                                                                          |
| `Media Consumerus\planning`                                     | BACKLOG.md rows → issues (priority/effort → priority/estimate); TODOS.md → issues with resolution log in description; full docs (BACKLOG/TODOS/DESIGN) → **doc tickets** (pages transformed 2026-09-09; Backlog/TODOS doc tickets deleted after content distribution, Design kept); `archive/v1` → **7 done Plan tickets** (e10-import/ code artifacts stay in repo) — **imported 2026-09-09 (Phase 5)**                                                                                                                                                                                                                                                       |
...
| `Empirium\pm\<app> folders`                                     | Task lists → issues in the respective app projects (or Personal if no project exists). **Remaining folder: Jobernaut (Phase 9)** — Arsenal imported 2026-09-09 (Phase 6), Don Saldo imported 2026-09-09 (Phase 7); Legaliosa / Media Consumerus / Kleinanzeigen / POP deleted 2026-09-09 (O13)                                                                                                                                                                                                                                 |
...
| `Empirium\pm\Arsenal` (Arsenal.md + improve-subagent-config.md) | Arsenal.md → 25 checkbox issues in Arsenal (label `todo`); improve-subagent-config → **doc ticket** (page → ticket 2026-09-09, §5.5) — **imported 2026-09-09 (Phase 6)**                                                                                                                                                                                                                                                                              |
| `Wisdomous\Arsenal\planning`                                    | Phase 6: 10 active plans → Plan issues (states per Karol 2026-09-09: plan-18×2 + plan-20×7 In Progress, plan-19 Backlog; incl. the `plan-20-audit-*.txt` files via `planFiles`); **HANDOFF → page** (Karol: a living doc, constantly updated — not a work item); done/ + archive/ → Plan issues, state Done (O4; only `.md` files; `plan-12-superseded-skills` + `plan-16-design-consolidation` subfolders **excluded** — archived skill content stays in the vault; `initial-repo-triage` **included** — Karol 2026-09-09) — **imported 2026-09-09 (Phase 6)**                                                                                                                                                                                                                                                             |
| `Wisdomous\Empirium\planning`                                   | Phase 6: 3 vault-migration plans → Plan issues (states per Karol 2026-09-09: Migration Plan In Progress, Phase1 Manifest + merge plan Done) — **imported 2026-09-09 (Phase 6)**                                                                                                                                                                                                                                                                      |
...
2. **Media Consumerus** (Phase 5 — exercises tables, sections, estimates, pages) — **done 2026-09-09** (22 issues: 10 backlog + 2 todos + 3 deferred + 7 archive plans; pages → tickets, doc tickets deleted after content distribution; importer fixes: §9.2)
3. **Arsenal** → **Empirium** (Phase 6 — vault planning; Kleinanzeigen + POP dropped 2026-09-09 — O13, content re-created manually) — **done 2026-09-09** (47 + 3 plans, 25 pm checkboxes + 1 doc ticket, HANDOFF page)
4. **Don Saldo** (Phase 7 — move to the last batch if it becomes actively developed) — **done 2026-09-09** (34 issues: 2 tickets + 2 doc tickets + 30 pm items)
5. **Legaliosa** (Phase 8 — **last**: in active development; Karol signals a good pause point first) — **done 2026-09-09** (281 issues + 86 relations + 6 pages; v1 archive imported as "Plan v1" hierarchy)
6. **Jobernaut** (Phase 9 — **last**: same) — **done 2026-09-09** (120 issues + 34 relations + 7 pages; "Plan v1"/"Plan v2" split; archive imported as Done; design skipped)
...
| **5. Media Consumerus**                                                                                                                  | BACKLOG table + TODOS sections → issues (estimates, labels), pages (the pm-folder config `media-consumerus-pm` is dropped — O13)                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Counts verified — **done 2026-09-09** (22 issues: 10 + 2 + 3 deferred + 7 archive plans; pages → tickets, doc tickets deleted; importer fixes, §9.2)          |
| **6. Arsenal → Empirium** (Kleinanzeigen + POP dropped 2026-09-09 — O13, re-created manually; their projects exist with Karol's tickets) | Vault planning (**tooling built in this phase**: `import-issues.js` gains a `planFiles` list (multiple plan files → Plan issues, incl. `.txt` — the walk only picks `.md`) + `donePlanDirs`/`archivePlanDirs` (each `.md` → Plan issue, state Done); new configs `arsenal-planning.json` + `empirium-planning.json`): active plans + HANDOFF → Plan issues; done/ + archive/ → Plan issues, state Done (O4); artifacts stay in vaults                                                                                                                                             | Counts verified — **done 2026-09-09** (47 Arsenal plans + 3 Empirium plans + 25 pm checkboxes + 1 doc ticket + HANDOFF page; states per Karol; superseded-skill subfolders excluded) |
| **7. Don Saldo**                                                                                                                         | 2 tickets + bank-import note (move to last batch if it becomes active)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Counts verified — **done 2026-09-09** (34 issues: 2 tickets + 2 doc tickets + 30 pm items; label-permission fix, §6.2; +13 docs/plans → Plan/In Progress)      |
| **8. Legaliosa** (last — Karol signals pause point)                                                                                      | Full hierarchy: Plan → 11 SP → 38 T → 211 tickets; relations, estimates, design reviews → pages; "Design" index page created in the UI                                                                                                                                                                                                                                                                                                                                                                                                                                            | Counts verified + spot-check — **done 2026-09-09** (281 issues + 6 pages + 86 relations; "Plan v1"/"Plan v2" split; archive imported as Done; design reviews skipped; SP sub_plan labels normalized)                            |
| **9. Jobernaut** (last — Karol signals pause point)                                                                                      | Same pattern from Jobernaut2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Counts verified — **done 2026-09-09** (120 issues + 7 pages + 34 relations; "Plan v1"/"Plan v2" split; archive imported as Done; design skipped; SP-13..16 labels) |
| **10. App integrations**                                                                                                                 | Report-issue module per app, bot tokens wired, **user-reports auto-assigned to Karol** (in-app ping), confirmation UX — **tracked as QUESTIMUS-38** (projects: Don Saldo, Jobernaut, Legaliosa, Questimus, Media Consumerus) with **QUESTIMUS-39 = Questimus app pilot** (mockup → code → template; decided 2026-09-09)                                                                                                                                                                                                                                                                                                           | Real test report lands in Questimus — **deferred** (Karol 2026-09-09: apps not fully built yet; Questimus pilot decided)                                  |
| **11. LLM skills**                                                                                                                       | Per-project skills in Arsenal, deployed to DSH/Claude Code/etc.; agent tokens wired                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Agent can triage + work + resolve via skill — **done 2026-09-09** (10 skills in the vault, deployed to 6 projects, 103/103 route tests pass; Personal/Kleinanzeigen/POP stay vault-only until repos exist) |
