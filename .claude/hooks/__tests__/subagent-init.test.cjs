...
describe("subagent-init.cjs", () => {
  describe("Basic Functionality", () => {
    ...
    it("returns valid JSON with hookSpecificOutput", async () => {
      ...
      assert.ok(result.output.hookSpecificOutput.additionalContext, "Should have additionalContext");
    });
  ...
  describe("Issue #540: ck plan CLI injection for plan-aware agents", () => {
    it("injects ck plan CLI section for agents that may update plan state", async () => {
      ...
      for (const agentType of planAwareTypes) {
        ...
        assert.ok(context.includes("Fallback"), `Agent type '${agentType}' should include fallback note`);
      }
    });
    ...
    it("does NOT inject ck plan CLI for non-plan agents", async () => {
      ...
      for (const agentType of nonPlanTypes) {
        ...
        const context = result.output?.hookSpecificOutput?.additionalContext || "";
        assert.ok(!context.includes("Plan CLI"), `Agent type '${agentType}' should NOT receive Plan CLI section`);
      }
    });
  });
  ...
  describe("Issue #291: CWD and Git Root Handling", () => {
    it("uses payload.cwd for context output", async () => {
      ...
      const context = result.output?.hookSpecificOutput?.additionalContext || "";
      assert.ok(context.includes(testCwd), `Should include payload.cwd in output: ${context}`);
    });
    ...
    it("falls back to process.cwd() when payload.cwd is undefined", async () => {
      ...
      const context = result.output?.hookSpecificOutput?.additionalContext || "";
      assert.ok(context.includes(process.cwd()), "Should fall back to process.cwd()");
    });
    ...
    it("resolves git root from payload.cwd, not process.cwd()", async () => {
      ...
      assert.ok(context.includes(gitRoot), `Context should include git root path: ${context}`);
    });
  ...
  describe("Context Output Validation", () => {
    ...
    it("injects descriptive report filename guidance", async () => {
      ...
      assert.match(context, /code-reviewer-[^\n]*-report\.md/, "Should include a report filename ending in -report.md");
      assert.match(
        context,
  ...
  describe("Advanced Git Scenarios", () => {
    ...
    it("handles git worktree", async () => {
      ...
      try {
        ...
        assert.ok(context.includes(worktreeDir), `Should include worktree path: ${context.substring(0, 200)}`);
    ...
    it("handles unicode characters in path", async () => {
      ...
      try {
        ...
        const context = result.output?.hookSpecificOutput?.additionalContext || "";
        assert.ok(context.includes("日本語") || context.includes("émoji"), "Should preserve unicode in output");
      } finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
      }
    });
  ...
  describe("Error Handling", () => {
    ...
    it("captures error in stderr on failure", async () => {
      ...
      if (stderr) {
        assert.ok(stderr.includes("error") || stderr.includes("Error"), "Error output should be informative");
      }
    });
  });
});
