  ...
  describe("Team Membership Detection", () => {
    it("detects team membership from agent_id (name@team-name pattern)", async () => {
      ...
      try {
        ...
        assert.ok(context.includes("Team Context"), "Should include Team Context section for team member");
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
    ...
    it("rejects malformed agent_ids for security (path traversal prevention)", async () => {
      ...
      try {
        ...
        for (const agentId of malformedIds) {
          ...
          assert.strictEqual(result.exitCode, 0, `Should safely reject malformed agent_id: ${agentId}`);
    ...
    it("handles empty or invalid agent_id gracefully", async () => {
      ...
      for (const testCase of testCases) {
        ...
        assert.strictEqual(result.exitCode, 0, `Should handle invalid agent_id: ${JSON.stringify(testCase)}`);
      }
    });
  });
  ...
  describe("Peer List Building", () => {
    it("builds peer list excluding current agent", async () => {
      ...
      try {
        ...
        assert.ok(context.includes("bob") && context.includes("charlie"), "Should list other team members");
        // Peer list should show type
        assert.ok(context.includes("tester") || context.includes("reviewer"), "Should include peer agent types");
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  describe("Task Summary Generation", () => {
    it("generates accurate task summary when tasks exist", async () => {
      ...
      try {
        ...
        assert.ok(context.includes("Task summary") || context.includes("pending"), "Should include task summary");
  ...
  describe("JSON Output Validation", () => {
    it("always outputs valid JSON with hookSpecificOutput structure", async () => {
      ...
      try {
        ...
        assert.ok(result.output, "Should return JSON output");
        assert.strictEqual(typeof result.output.hookSpecificOutput, "object", "Should have hookSpecificOutput object");
        assert.strictEqual(
          result.output.hookSpecificOutput.hookEventName,
          "SubagentStart",
