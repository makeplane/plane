...
describe("session-init.cjs", () => {
  describe("Issue #277 Mitigation: Compact Warning", () => {
    it("outputs warning when source=compact", async () => {
      ...
      assert.ok(result.stdout.includes("⚠️ CONTEXT COMPACTED - APPROVAL STATE CHECK"), "Should include warning header");
      assert.ok(result.stdout.includes("AskUserQuestion"), "Should mention AskUserQuestion");
      assert.ok(result.stdout.includes("MUST re-confirm"), "Should instruct to re-confirm with user");
    });
    ...
    it("does NOT output warning when source=startup", async () => {
      ...
      assert.strictEqual(result.exitCode, 0, "Hook should exit with code 0");
      assert.ok(!result.stdout.includes("⚠️ CONTEXT COMPACTED"), "Should NOT include compact warning for startup");
    });
    ...
    it("does NOT output warning when source=resume", async () => {
      ...
      assert.strictEqual(result.exitCode, 0, "Hook should exit with code 0");
      assert.ok(!result.stdout.includes("⚠️ CONTEXT COMPACTED"), "Should NOT include compact warning for resume");
    });
    ...
    it("does NOT output warning when source=clear", async () => {
      ...
      assert.strictEqual(result.exitCode, 0, "Hook should exit with code 0");
      assert.ok(!result.stdout.includes("⚠️ CONTEXT COMPACTED"), "Should NOT include compact warning for clear");
    });
    ...
    it("handles empty stdin gracefully", async () => {
      ...
      assert.strictEqual(result.exitCode, 0, "Hook should exit with code 0");
      assert.ok(!result.stdout.includes("⚠️ CONTEXT COMPACTED"), "Should NOT include compact warning for empty stdin");
    });
  });
  ...
  describe("Warning Content Validation", () => {
    it("warning contains all required guidance elements", async () => {
      ...
      assert.ok(result.stdout.includes("Step 4 review gate"), "Should mention Step 4 review gate as example");
      ...
      assert.ok(result.stdout.includes("Do NOT assume approval was given"), "Should warn against assuming approval");

      // Line 4: Action
      assert.ok(result.stdout.includes("Use AskUserQuestion to verify"), "Should instruct to use AskUserQuestion");
      assert.ok(
  ...
  describe("Session Context Output", () => {
    it("includes session source in output", async () => {
      ...
      assert.ok(result.stdout.includes("Session compact"), 'Should output "Session compact" at start');
    });
  ...
  describe("Issue #291: Git Root Path Resolution", () => {
    ...
    it("shows subdirectory info when CWD differs from git root (Issue #327)", async () => {
      ...
      assert.ok(result.stdout.includes("Subdirectory mode"), "Should show subdirectory info when not at git root");
