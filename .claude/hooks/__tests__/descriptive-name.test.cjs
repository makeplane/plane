...
describe("descriptive-name.cjs", () => {
  describe("Hook output structure", () => {
    ...
    it("has correct hookEventName", async () => {
      ...
      assert.strictEqual(parsed.hookSpecificOutput.hookEventName, "PreToolUse", "hookEventName should be PreToolUse");
    });

    it("always allows operation (permissionDecision: allow)", async () => {
      const { parsed } = await runHook();

      assert.strictEqual(parsed.hookSpecificOutput.permissionDecision, "allow", "Should always allow Write operations");
    });

    it("includes additionalContext with guidance", async () => {
      const { parsed } = await runHook();

      assert.ok(parsed.hookSpecificOutput.additionalContext, "Should include additionalContext");
  ...
  describe("Language-aware guidance content (Issue #440)", () => {
    it("mentions kebab-case preference for JS/TS/shell", async () => {
      ...
      assert.ok(context.includes(".sh"), "Should mention .sh extension for shell scripts");
      assert.ok(!context.includes("JS/TS/Python"), "Should not group Python into the kebab-case preference");
    });
    ...
    it("respects C#/Java/Kotlin/Swift PascalCase convention", async () => {
      ...
      assert.ok(context.includes("PascalCase"), "Should mention PascalCase for C#/Java/Kotlin/Swift");
      assert.ok(context.includes(".cs"), "Should mention .cs extension for C#");
      assert.ok(context.includes(".java"), "Should mention .java extension for Java");
      assert.ok(context.includes(".kt"), "Should mention .kt extension for Kotlin");
      assert.ok(context.includes(".swift"), "Should mention .swift extension for Swift");
    });
    ...
    it("respects Python/Go/Rust snake_case convention", async () => {
      ...
      assert.ok(context.includes("snake_case"), "Should mention snake_case for Python/Go/Rust");
      assert.ok(context.includes(".py") || context.includes("Python"), "Should mention Python extension or language");
      assert.ok(context.includes(".go") || context.includes("Go"), "Should mention Go extension or language");
      assert.ok(context.includes(".rs") || context.includes("Rust"), "Should mention Rust extension or language");
    });
    ...
    it("does NOT use strict MUST language", async () => {
      ...
      assert.ok(!context.includes("must use kebab"), 'Should NOT use strict "MUST use kebab-case" language');
    });
    ...
    it('uses soft "prefer" language for kebab-case', async () => {
      ...
      assert.ok(context.includes("prefer"), 'Should use soft "prefer" language for kebab-case');
    });
    ...
    it("guides Markdown report names without skipping them", async () => {
      ...
      assert.match(context, /Avoid generic report names/i, "Should discourage generic report names");
    });
  });
  ...
  describe("Hook disable functionality", () => {
    ...
    it("exits silently when disabled via .ck.json", async () => {
      ...
      fs.writeFileSync(path.join(claudeDir, ".ck.json"), JSON.stringify({ hooks: { "descriptive-name": false } }));
  ...
  describe("Error handling", () => {
    it("fails open on unexpected errors (exit 0)", async () => {
      ...
      assert.ok(hookContent.includes("catch (error)"), "Hook should have error handling");
      assert.ok(hookContent.includes("process.exit(0)"), "Hook should fail open (exit 0) on errors");
    });
  });
});
