...
try {
  ...
  const PLAN_AWARE_AGENTS = new Set(["planner", "project-manager", "code-simplifier", "fullstack-developer"]);
  ...
  async function main() {
    ...
    try {
      ...
      lines.push(
        `- For workflow reports, insert a descriptive purpose before -report, e.g. ${agentType}-${namePattern}-red-team-plan-review-report.md`
      );
