...
function assertContains(actual, expected, message) {
  if (!actual.includes(expected)) {
    throw new Error(`${message || "Missing expected string"}\nExpected: ${expected}\nActual: ${actual}`);
  }
}
...
function createTempConfigProject(mode) {
  ...
  fs.writeFileSync(path.join(ckDir, ".ck.json"), JSON.stringify({ statusline: mode }, null, 2));
  return tmpDir;
}
  ...
  await test("Usage cache available shows reset countdown and utilization", async () => {
    withUsageCache(
      ...
      () => {
        ...
        assertTrue(
          /\(38%/.test(stdout) || /\(38 used\)/.test(stdout) || /\(38/.test(stdout),
          "Should include rounded utilization"
        );
      }
    );
  });
