...
try {
  const costResult = execSync(
    `echo '${costInput.replace(/'/g, "'\\''")}'  | CLAUDE_BILLING_MODE=api ${STATUSLINE_CMD}`,
    {
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
...
try {
  ...
  test("Agent tracking shows completed agent", () => {
    ...
    const hasAgent =
      agentTodoResult.includes("researcher") || agentTodoResult.includes("○") || agentTodoResult.includes("●");
    assertTrue(hasAgent, "Should show agent info");
  });

  test("Todo tracking shows in-progress task", () => {
    const hasTodo =
      agentTodoResult.includes("▸") || agentTodoResult.includes("second") || agentTodoResult.includes("Working");
