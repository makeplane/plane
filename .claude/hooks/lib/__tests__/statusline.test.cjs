...
const { countConfigs, countRulesInDir, countMcpServersInFile, countHooksInFile } = require("../config-counter.cjs");
...
test("green() wraps text with color codes or returns plain text", () => {
  ...
  assertTrue(result === text || result.includes(text), "green() should return colored or plain text");
});
...
test("getGitInfo timeout reuses stale cache when available", () => {
  ...
  try {
    fs.writeFileSync(cachePath, JSON.stringify({ timestamp: Date.now() - 120000, data: staleData }));
    process.env.PATH = `${fakeBinDir}:${originalPath || ""}`;
    process.env.CK_GIT_TIMEOUT_MS = "50";
