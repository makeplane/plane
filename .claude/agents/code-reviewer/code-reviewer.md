...
- Use Bash read-only (grep, git log, git diff) to gather context. Never write. The `Agent` tool grant exists for exactly one purpose — dispatching specialist subagents per Specialist Dispatch above — never use it to delegate the review itself or any other part of your own job.
