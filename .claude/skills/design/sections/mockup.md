...
4. **Write the HTML.** Continue at `design/<screen>/vN.html` until settled. Always include:
   - Local vendored libs — **no CDN**: `<script type="module">import { prepare, layout, prepareWithSegments, walkLineRanges, layoutNextLine, layoutWithLines, clearCache, setLocale } from '<skill-root>/assets/pretext/pretext.js'</script>` and, for charts, `<script src="<skill-root>/assets/chart/chart.umd.js"></script>` (Chart.js 4.4.3, UMD, sets `window.Chart`). `<skill-root>` is `.claude/skills/design` in project-deployed projects.
