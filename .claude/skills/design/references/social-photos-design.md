...
- **Brand context** — read from `design/brand-guidelines.md` if exists
...
Run the design skill's modes in sequence:

1. **`/design brand`** — Extract brand colors, fonts, voice from user's project
2. **`/design design-system`** — Get design tokens (spacing, typography scale, color palette)
3. **`/design mockup`** — for layout, hierarchy, visual balance (the mockup mode's Explore step covers variant variety).
...
- **Self-contained** — Inline all CSS, use a font from the design skill's vendored `assets/fonts/` kit (copy it beside this HTML) or a system font stack — no CDN at render
...
<head>
  ...
  <!-- Font: no CDN at render. Use a font from the design skill's vendored
       assets/fonts/ kit (copy it beside this HTML) or a system font stack. -->
  <style>
...
5. Save to `design/social-photos/exports/`
