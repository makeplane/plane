...
function parseFormat0(content, dir, options) {
  ...
  while (i < lines.length) {
    ...
    let phaseCol = -1,
      nameCol = -1,
      nameColExplicit = false,
      statusCol = -1;
    headers.forEach((h, idx) => {
      if (h === "#" || h === "phase" || h === "id") {
        if (phaseCol < 0) phaseCol = idx;
      }
      ...
      if (h === "phase" && idx !== phaseCol && !nameColExplicit) {
        if (nameCol < 0) nameCol = idx;
      }
...
function parsePlanPhases(content, dir, options = {}) {
  ...
  while ((match = f2Regex.exec(content)) !== null) {
    phases.push(
      makePhase(match[1], match[2], match[4], match[5], match[3], `Phase ${match[1]}${match[2]}`, dir, options)
    );
  }
  if (phases.length > 0) return phases;
  ...
  if (/^-\s*Phase\s*\d+[:\s]/m.test(content)) {
    ...
    for (const line of lines4) {
      ...
      if (cur4) {
        const fm = /^\s+-\s*File:\s*`?([^`\n]+)`?/i.exec(line);
        if (fm) {
          cur4.file = path.resolve(dir, fm[1].trim());
          cur4.anchor = null;
        }
  ...
  if (phaseMap.size > 0) {
    ...
    phases.push(
      ...Array.from(phaseMap.values()).sort((a, b) => a.phase - b.phase || a.phaseId.localeCompare(b.phaseId))
    );
    return phases;
  }

  // Format 6: "- [ ] **[Phase 1: Name](path)**"
  const f6Regex = /^-\s*\[(x| )\]\s*\*\*\[(?:Phase\s*)?(\d+)([a-z]?)[:\s]*([^\]]*)\]\(([^)]+)\)\*\*/gim;
  while ((match = f6Regex.exec(content)) !== null) {
    const name = match[4].trim() || `Phase ${match[2]}${match[3]}`;
    phases.push(
      makePhase(match[2], match[3], name, match[1] === "x" ? "completed" : "pending", match[5], name, dir, options)
    );
  }
