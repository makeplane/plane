...
- Syncing after `pnpm --filter @workspaces/i18n run sync:check` reports drift
...
### The brand name collides with a feature noun — read this first

The product is called **Workspaces**, and the product also contains an entity called a
**workspace**. They are different things and they translate differently:

- **`Workspaces` — the product name.** A brand mark. Latin in every locale, always capitalised,
  never translated. "Welcome to Workspaces", "Sign in to Workspaces", "your Workspaces account".
- **`workspace` / `workspaces` — the entity a user creates and invites people into.** An ordinary
  common noun. **Translate it** (zh-CN 工作区, ja ワークスペース, ru рабочее пространство,
  de Arbeitsbereich, es espacio de trabajo, fr espace de travail …), exactly as before the rename.

Decide by sense, not by capitalisation alone — English source strings are not always reliable here.

```
✅ "Bienvenido a Workspaces"                        (product name → Latin)
✅ "Crea un espacio de trabajo"                     ("create a workspace" → common noun, translated)
✅ "Tu cuenta de Workspaces"                        ("your Workspaces account" → product)
✅ "Invita a miembros a tu espacio de trabajo"      ("your workspace" → entity)
❌ "Bienvenido a Espacios de trabajo"               (translated the product name)
❌ "Crea un Workspaces"                             (left the entity noun Latin)
```

A string can contain both: "Connect your Slack workspace with Workspaces" → the first is the
entity, the second is the product.
...
| ------------------------------------ | ----------------------------------------------------------------------- | --------------------- | ------------------- | --------------------------------------------- | ----------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **Workspaces** (product name)        | Workspaces                                                              | Workspaces            | Workspaces          | Workspaces                                    | Workspaces        | Workspaces        | 工作区, 工作區, ワークスペース, 작업 공간, Рабочие пространства, Робочі простори, Espacios de trabajo, Espaces de travail, Arbeitsbereiche |
| **Workspaces AI** (formerly PI Chat) | Workspaces AI                                                           | Workspaces AI         | Workspaces AI       | Workspaces AI                                 | Workspaces AI     | Workspaces AI     | Чат ИИ, AI 聊天, AIチャット, AI 채팅, Chat IA, AI Çet, PI Chat (legacy)                                                                    |
...
### Workspaces feature noun translation glossary (translate, do not preserve)
...
- **Generic uses translate normally and don't follow this glossary:** `next page` (paginator), `the page` (browser refresh), `status page`, `web page`, `life cycle`, `release cycle`, `rate-limit cycle`, `cycle of releases` — these are not the Workspaces Cycle/Page feature; translate them as ordinary words in the surrounding prose.
...
When a Slavic plural block counts a Workspaces feature noun, use these forms:
...
**Translate Workspaces feature nouns** (Cycle, Cycles, Module, Modules, Epic, Epics, Page, Pages) using the per-locale form from the glossary above. **Keep Latin** for Workspaces brand marks (Workspaces, Workspaces AI, Power K, PQL, Active Cycles, Sticky, Stickies, Intake), plan tier names (Pro, Business, Enterprise), and third-party brands (GitHub, Slack, etc.).
...
✅ "Buat Sticky baru"                       (id — Sticky is a Workspaces brand mark, stays Latin)
...
Generic uses translate normally and do not follow the glossary: a paginator's `next page` is `nächste Seite` / `página siguiente` (generic noun, not the Workspaces Pages feature). The glossary applies only when EN refers to the Workspaces product feature.

### Japanese (ja) — natural Japanese rendering

**Workspaces feature nouns** (Cycle, Module, Epic, Page) use the natural Japanese form per the glossary above (サイクル, モジュール, エピック, ページ — katakana for foreign-origin nouns; native Japanese where one applies, like 付箋 for an Apple/Microsoft-style "sticky note"). **Brand marks** stay in Latin: Workspaces, Workspaces AI, Power K, PQL, Active Cycles, Sticky, Stickies, Intake, GitHub, Slack, Pro/Business/Enterprise.
...
**Workspaces feature nouns** (Cycle, Module, Epic, Page) use the natural Korean form per the glossary above (사이클, 모듈, 에픽, 페이지 — Hangul transliteration where the term is product-coined; native Korean word where one applies). **Brand marks** stay in Latin: Workspaces, Workspaces AI, Power K, PQL, Active Cycles, Sticky, Stickies, Intake, GitHub, Slack, Pro/Business/Enterprise.
...
**Workspaces feature nouns** (Cycle, Module, Epic, Page) translate to natural Chinese per the glossary above (zh-CN: 周期, 模块, 史诗, 页面 — zh-TW: 週期, 模組, 史詩, 頁面). This is what Microsoft's zh-CN style guide and every mainstream zh-localized SaaS product (Notion, Slack, Atlassian) does for common feature nouns.

**Brand marks stay Latin**: Workspaces, Workspaces AI, Power K, PQL, Active Cycles, Sticky, Stickies, Intake, GitHub, Slack, Pro/Business/Enterprise — these are trademark-style terms.
...
✅ "归档此史诗"                  (Workspaces Epic → 史诗 per glossary)
...
Register: 您 (formal polite) in the Workspaces UI — the product is B2B/SaaS. Reserve 你 for consumer/youth contexts (not applicable here).

### Cyrillic locales

Currently includes ru, ua — and any future Cyrillic locale (bg-BG, sr-Cyrl, mk, be, kk-Cyrl, etc.).

**Workspaces feature nouns** (Cycle, Module, Epic, Page) use the natural Cyrillic form per the glossary above (Цикл/Циклы, Модуль/Модули, Эпик/Эпики, Страница/Страницы in ru — Цикл/Цикли, Модуль/Модулі, Епік/Епіки, Сторінка/Сторінки in ua). **Brand marks** stay in Latin: Workspaces, Workspaces AI, Power K, PQL, Active Cycles, Sticky, Stickies, Intake, GitHub, Slack, Pro/Business/Enterprise.
...
4. Run `pnpm --filter @workspaces/i18n run generate:types` (or let the build do it).
5. Run `pnpm --filter @workspaces/i18n run sync:check` — expect `0 missing, 0 stale, 0 collisions`.
...
5. Run `pnpm --filter @workspaces/i18n run generate:types` to regenerate the `TTranslationKeys` union so component-level `t()` calls type-check.
6. Run `pnpm --filter @workspaces/i18n run sync:check` — expect `0 missing, 0 stale, 0 collisions`.
...
5. Before merging, **pseudolocalize**: temporarily replace the new locale's values with bracketed expanded forms (`"Workspaces" → "[Wörkspàcéés——]"`) in a local build and click through the UI. Catches truncation, unextracted strings, and layout bugs before real users see them.
...
pnpm --filter @workspaces/i18n run generate:types

# Report drift
pnpm --filter @workspaces/i18n run sync:check

# Same, exit 1 on drift (for CI)
pnpm --filter @workspaces/i18n run check:sync
```
...
| Question                                                      | Answer                                                                                                                                                                 |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Translate "Workspaces" / "Workspaces AI" / "Power K" / "PQL"? | Never. Latin, every locale.                                                                                                                                            |
| Translate "Sticky" / "Stickies" / "Intake"?                   | Never. Workspaces brand marks. Latin, every locale.                                                                                                                    |
...
- **Translating Workspaces brand marks** — `Workspaces → 工作区`, `Workspaces AI → AI 聊天`, `Power K → 命令面板`, `Sticky → 便签` (Sticky is a brand, even though "sticky note" generally translates), `Intake → 收件箱`. Brand marks stay Latin.
...
- **Using informal pronouns** — `du/tu/ты/tú/you (informal)` breaks the Workspaces formal register in languages that distinguish.
...
- Replacing `Workspaces`, `Workspaces AI`, `Power K`, `PQL`, `Active Cycles`, `Sticky`, `Stickies`, `Intake`, `Pro`, `Business`, `Enterprise`, or any third-party brand (`GitHub`, `Slack`, etc.) with a translated form.
...
| "I'll just transliterate the Workspaces brand name into Cyrillic for accessibility." | Brand marks stay Latin in every locale. `Воркспейсес` is wrong; `Workspaces` is right.                                                                                         |
...
| "Capitalizing `Anda` / `您` / `Sie` looks over-formal."                              | It's the Workspaces register in those languages. Deviating breaks brand voice across the product.                                                                              |
...
6. **DNT glossary** — Add a column (or row entries) for the new locale across the DNT tables. Specify forbidden literal translations (the words an LLM would produce if uninstructed). Example: if adding Arabic, the forbidden rendering for `Workspaces` includes `مساحات العمل`; for `Epic` includes `ملحمة`; for `Sticky` includes `ملاحظة`.
