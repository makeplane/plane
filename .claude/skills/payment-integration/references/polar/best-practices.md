...
export async function POST(request: Request) {
  try {
    ...
    if (data.couponCode) {
      const couponResult = await validateAndApplyCoupon(data.couponCode, productId, originalAmount);
    ...
    if (data.referralCode) {
      ...
      if (referralResult.valid && referralResult.discountAmount > 0) {
        ...
        if (referralResult.discountAmount <= 0) {
          return NextResponse.json({ error: "Invalid discount calculation - contact support" }, { status: 400 });
        }
...
export async function POST(request: Request) {
  ...
  const existingEvent = await db.select().from(webhookEvents).where(eq(webhookEvents.eventId, eventId)).limit(1);
...
export function calculatePolarFees(
  ...
  const internationalFee = isInternational ? Math.round(amountCents * POLAR_FEES.internationalSurcharge) : 0;
  const subscriptionFee = isSubscription ? Math.round(amountCents * POLAR_FEES.subscriptionSurcharge) : 0;
...
export async function validateDiscount(
  ...
  try {
    ...
    // Find exact match
    const discount = result.items.find((d) => d.code?.toUpperCase() === sanitizedCode);
