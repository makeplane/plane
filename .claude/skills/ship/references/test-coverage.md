...
After producing the coverage diagram, write a test plan artifact so `review-fix-technical-code`'s App-QA mode and `/qa-report-only` can consume it:
...
2. Store `coverage_pct` (for PR body), `gaps` (user summary), `tests_added` (for the commit).
