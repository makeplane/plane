...
export const DataDisplay: React.FC<{ items: Item[]; searchTerm: string }> = ({ items, searchTerm }) => {
...
export const ExpensiveComponent = React.memo<ExpensiveComponentProps>(function ExpensiveComponent({ data, onAction }) {
...
const ListItem = React.memo<ListItemProps>(({ item, onAction }) => {
  return <Box onClick={() => onAction(item.id)}>{item.name}</Box>;
});
