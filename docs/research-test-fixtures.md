...
工作区：`fangyikai`（<http://192.168.3.245:3000/fangyikai/research/>）。夹具账号统一初始密码
**`Research@12345`**，邮箱域名 `@ai4ms.local`，仅用于本地无 SMTP / 无 OIDC 的测试环境。

## 2. 账号速查表

`fangyikai`（`fangyikaii@163.com`）保持原密码不变，只补齐科研角色：工作区管理员 + 学院主 PI +
`AI4MS 计算材料课题组` 主 PI + 阶段评审人 + 审批人。

| 邮箱                            | 姓名 | 身份与组织位置                                       | 预期能看到什么                                                    |
| ------------------------------- | ---- | ---------------------------------------------------- | ----------------------------------------------------------------- |
| `zhangwei.pi@ai4ms.local`       | 张伟 | 张伟课题组主 PI（primary）+ 科研项目负责人           | 本课题组全部报告/阶段/审批；被自动指派为本组阶段评审人            |
| `liming.pi@ai4ms.local`         | 李明 | 李明课题组主 PI（primary）                           | 自己课题组数据；看不到张伟课题组的 PRIVATE / 导师级记录           |
| `wangfang.lab@ai4ms.local`      | 王芳 | 能源材料实验室负责人（OWNER）+ 王芳课题组主 PI       | 实验室下课题组的 `ANCESTRY` 级记录（非管理员，验证上级节点主 PI） |
| `zhaoqiang.admin@ai4ms.local`   | 赵强 | 学院组织管理员（UNIT_ADMIN）                         | 组织架构管理入口；学院范围的 `DIRECT_ADVISOR` / `UNIT` 记录       |
| `chenjing.advisor@ai4ms.local`  | 陈静 | 直接导师（ADVISOR，张伟课题组）                      | 被绑定学生（刘洋 / 周敏 / 孙浩）的全部报告；阶段必评人            |
| `zhengkai.reviewer@ai4ms.local` | 郑凯 | 评审人（张伟课题组）+ 石墨负极小组负责人             | 被指派的阶段评审；本小组记录                                      |
| `liuyang.phd@ai4ms.local`       | 刘洋 | 博士生（张伟课题组）                                 | 自己项目全链路数据 + 6 篇不同可见范围的报告（ACL 矩阵所有者）     |
| `zhoumin.master@ai4ms.local`    | 周敏 | 硕士生（张伟课题组）                                 | 被显式授权的 `CUSTOM` 报告；同课题组 `UNIT` 级记录                |
| `sunhao.postdoc@ai4ms.local`    | 孙浩 | 博士后（张伟课题组）                                 | 自己的项目：预开题已通过、开题门槛全绿可现场提交                  |
| `wuting.project@ai4ms.local`    | 吴婷 | 科研项目人员（AI4MS 计算材料课题组，导师=fangyikai） | 自己的项目：预开题被退回（带退回原因），报告直接导师是 fangyikai  |
...
└── 材料科学与工程学院（INSTITUTE）
    ├── 能源材料实验室（LAB）
    │   ├── 张伟课题组（GROUP）—— 主 PI 张伟；下辖 石墨负极小组（TEAM，郑凯）
    │   └── 李明课题组（GROUP）—— 主 PI 李明
    └── 智能材料实验室（LAB）
        ├── 王芳课题组（GROUP）—— 主 PI 王芳
        └── AI4MS 计算材料课题组（GROUP）—— 主 PI fangyikai
```

共 5 层，用于验证层级可见范围、祖宗节点主 PI 与组织架构页面的树形交互。
...
- `owner` = 刘洋（矩阵报告作者）；`advisor` = 陈静（MentorBinding）；`unit_member` = 周敏（同课题组）；
  `ancestor_pi` = 王芳（上级实验室主 PI，非管理员）；`admin` = fangyikai；`stranger` = 高鹏。
...
6. **组织架构**（`/research/settings/org`）：确认 5 层组织树、成员角色（PI / ADVISOR / REVIEWER /
