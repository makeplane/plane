...
./scripts/gitnexus.sh pull              # tải image (~500MB)
...
| Disk space     | ~1 GB      | Image ~500MB + index ~150MB                   |
...
Mặc định: `akonlabs/gitnexus:1.6.4-rc.63` (~500MB). Override bằng env: `GITNEXUS_IMAGE=...`.
...
Thêm vào `.gitnexusignore` (cú pháp giống `.gitignore`):
