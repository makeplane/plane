...
|---------|------|------|-------------|------------------|
| 0.1.1 | 2026-07-11 | fix | Crash DeepMap arité : 3 appels internes de `getProjectIssueTypes` passent désormais `activeOnly=false` explicite (arité constante = 2, imposée par `computedFn`/`DeepMap` MobX). | apps/web/core/store/issue-type.store.ts |
