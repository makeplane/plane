...
| 复核结论 | **通过**（2026-09-16，v1.1）：P1 十四个阶段全部交付，145 条需求编号全部有实现与测试，见 §11                   |
...
| D-06 | §4.2 材料状态 `DRAFT/SUBMITTED/ACCEPTED/REJECTED` 未定义驱动事件，§3.4 只说"提交后只读"                    | 材料状态由阶段流转驱动：阶段 `SUBMITTED` → 材料 `SUBMITTED`；`PASSED` → `ACCEPTED`；`NEEDS_REVISION` / `REOPEN` → 材料回到 `DRAFT`（交还作者修改）；`REJECTED` 留给材料被显式否决的场景，编辑一次即回到 `DRAFT`；gate 的 `material_set` 以 `DRAFT/SUBMITTED/ACCEPTED` 计为齐备 |
...
**通过。P1 十四个开发阶段全部交付并通过验证。**

| 复核项   | 结果                                                                       |
| -------- | -------------------------------------------------------------------------- |
| 需求覆盖 | 145 条编号全部有实现落点与测试（PRD §16.3 实现索引）                       |
| 阶段交付 | A1 / A2 / B1–B4 / C1–C2 / D1–D4 / E1 / E2 共 14 个阶段，分阶段提交 commit  |
| 后端测试 | `pytest plane/tests` **1029 passed**；科研专项 605 passed                  |
| 安全回归 | Page 旁路、跨 Workspace、快照 ACL、凭证只写、审计不可改，全部通过          |
| 性能守护 | gate < 2s、timeline / progress < 5s（120 条实验 + 120 条文献 + 60 条制品） |
| 前端     | 类型检查通过、OxLint 0 error、中英 key 一致（`research.*` 457 个 key）     |
| 版本号   | `2.1.0 → 2.2.0`，根 / web / api / ui 同步                                  |
| 回写     | §10 清单的全部条目已落到 PRD v1.1（§16 实现回写记录）                      |

实现期间新增的决定（D-15 ~ D-20、T-16 ~ T-17）已在 PRD §16.1 / §16.2 登记，评审原意未变。
...
| v1.1 | 2026-09-16 | 实现复核：补记 D-15 ~ D-20、T-16 ~ T-17 的落地方式，给出验收与发布门禁结果，结论改为通过 | —    |
