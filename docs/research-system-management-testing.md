...
1. 准备一个 CSV（表头：`姓名,学号,邮件,手机号,年级,人员类别,业务方向,小组,主导师,联合导师1,联合导师2`），例如：

   ```csv
   姓名,学号,邮件,手机号,年级,人员类别,业务方向,小组,主导师,联合导师1,联合导师2
   测试学生甲,20260000000011,manual.student1@stu.xmu.edu.cn,17000000011,25,学生,基础研究,石墨负极小组,刘俊扬,,
   测试学生乙,20260000000012,manual.student2@stu.xmu.edu.cn,17000000012,25,学生,基础研究,石墨负极小组,陈志昕,,
   ```

2. 另准备导师表：`姓名,邮箱` + `刘俊扬,manual.advisor@xmu.edu.cn`、`陈志昕,manual.advisor2@xmu.edu.cn`。
3. 打开 `/public/research/settings/system` → 「批量导入」上传两个文件 → 先点「预检」：
   - 预检结果列出逐行状态，**不写库**；
4. 再点「正式导入」→ 下载「导入报告」：
   - 两行均可在导师映射和 TEAM 小组存在时导入；缺少映射或小组不唯一的行会标记待处理；
   - 再次导入同一文件 → 不重复建号（幂等），一次性密码不再重发。
5. 校验：学生挂到已有「石墨负极小组」TEAM 节点；首次登录时弹出「请设置你自己的密码」强制改密弹窗。
