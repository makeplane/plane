...
用户导入和邀请码注册仍沿用 v2.4 契约，本版只保留兼容边界，不修改它们的字段、页面、注册流程和导入规则。
...
当前 v3 契约只提供引用元数据，不返回报告、Page 或外部系统正文，也没有写入、流程执行或模型调用能力。每次读取写入 `context.read` 审计事件，审计元数据只记录是否使用项目筛选和返回数量，不记录正文、API Token 或其他凭证。
...
- `ResearchInviteCode`、注册错误码、邀请注册链接和原子消耗规则保持 v2.4 契约。
- `UserImportBatch / UserImportRow / ResearchUserProfile`、CSV 列、预检、正式导入、初始密码和幂等规则保持不变。
- 账号生命周期接口保留 `is_account_compat_admin` 兼容检查；已有职责标签调用方在同时满足工作空间成员和导航门槛时仍可使用邀请及导入流程。这个例外只存在于账号导入/邀请码接口，不能外推为组织配置或科研内容权限。
- 不新增会阻塞伙伴开发的必填字段；旧 `GROUP`、`REVIEWER` 和非主归属关系仍可写入并由 v3 解析层兼容。
