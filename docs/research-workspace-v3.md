...
- 新单表名册固定包含 `姓名`、`学号`、`邮件`、`手机号`、`年级`、`人员类别`、`业务方向`、`小组`、`主导师`、`联合导师1`、`联合导师2`；小组只匹配已有 `TEAM` 节点，导师姓名通过必传的姓名→邮箱表解析。
...
- 新二维表、导师映射文件、预检、正式导入、初始密码和报告下载契约继续工作；旧二维表字段不再接受。学生表或导师表一旦包含当前工作区未软删除的成员，整批立即拒绝，已有成员资料应通过成员管理修改。学生仍可写入 `REVIEWER`。
