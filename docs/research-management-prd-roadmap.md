...
### 4.3.1 科研目录分级可见（v2.5.0）

导航入口按「工作区子开关 × 用户级别」两层判定，级别由既有组织关系推导，不新增身份字段：

| 级别         | 判定依据                                             | 额外菜单                                               |
| ------------ | ---------------------------------------------------- | ------------------------------------------------------ |
| `ADMIN`      | 实例管理员标签或工作区管理员                         | 系统管理、报告模板、身份映射、平台配置、审计、系统集成 |
| `PRINCIPAL`  | 有效组织角色 `OWNER` / `PI` / `UNIT_ADMIN`           | 主PI看板、提交汇总、组织架构（仅本节点及子树可写）     |
| `MENTOR`     | 组织角色 `ADVISOR`、有效导师绑定或被指派的阶段评审人 | 提交汇总、待我评审                                     |
| `RESEARCHER` | 其它组织关系（学生席位为 `REVIEWER`）或科研档案      | 仅科研总览、报告、科研项目、办公审批                   |
| `NONE`       | 无任何科研关系（如刚自动开通的账号）                 | 不显示科研目录，接口一律 403                           |

规则：

- `待我评审` 按人判定：被指派为评审人时，最低档也能看到。
- 项目内页（阶段、文献、实验、代码、成果、时间线）不按级别收口，仍由项目 ACL 决定。
- 级别只收口入口，进入后的数据范围仍由 §4.2 / §4.3 的对象 ACL 决定。
- 主PI工作区不持有组织树，其级别按公共工作区的组织关系解析。
- 菜单与接口同源：后端计算、接口强制、前端只渲染，详见 `research-navigation-visibility.md`。
...
| 模块     | P2 范围                                                                                                                                      |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 系统管理 | 双工作区（公共 / 主PI）、管理员标签、邀请码注册、二维表批量导入，见 [research-system-management-prd.md](./research-system-management-prd.md) |
