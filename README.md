...
当前仓库以 Plane `1.4.x` 的功能为基线，并已完成第一轮私有化清理：移除付费套餐与升级入口、上游遥测与版本检查、Plane 营销与帮助外链，以及不可用的 GitHub/Slack/Jira 集成入口；工作区版本已升到 `2.0.0`。尚未包含 AI4MS 专用集成代码，README 中的平台融合内容代表目标架构和实施路线，不表示相关能力已经上线。
...
![Plane for AI4MS 完善后的集成方案](docs/ai4ms-plane-enhanced-integration.png)

完善方案进一步补充了三部分内容：**用户身份映射与统一登录**、**科研过程记录与汇报体系**、**响应式 Mobile Web 交付策略**。这些仍是规划中的目标方案，当前代码尚未实现对应集成。
...
方案草图中的“复用 Next.js”表示复用现有 Web 技术栈的意图；本仓库当前 Web 端实际采用 React Router v7 + Vite，并保留 Next 兼容层，实施时以现有技术栈为准。
