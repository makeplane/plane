...
| 项目             | 内容                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------ |
| 当前版本         | `2.3.0`（根 / `apps/web` / `apps/api` / `packages/ui` 同步）                         |
| 上游基线         | Plane `1.4.x`                                                                        |
| 已交付           | 科研管理 P0（`2.1.0`）、阶段流程与集成 P1（`2.2.0`）、科研测试夹具（`2.3.0`）        |
...
| Plane for AI4MS | 科研及办公管理模块                             | 承接任务、周期、模块、页面、任务审批、采购审批、报告和复盘 | 已交付 P0 / P1            |
...
2. 通过 OAuth2/OIDC 将身份上下文传递给 Plane for AI4MS。
