...
export const getSlashCommandFilteredSections =
  ...
  ({ query }: { query: string }): TSlashCommandSection[] => {
    const slashCommandSections: TSlashCommandSection[] = [
      {
        key: "general",
        items: [
          {
    ...
    if (additionalOptions) {
      additionalOptions.map((item) => slashCommandSections?.[0]?.items.push(item));
    }

    const filteredSlashSections = slashCommandSections.map((section) => ({
      ...section,
