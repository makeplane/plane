...
export const AIHandlePlugin = (options: SideMenuPluginProps): SideMenuHandleOptions => {
  ...
  const view = (view: EditorView, sideMenu: HTMLDivElement | null) => {
    ...
    const className =
      "grid place-items-center font-medium size-5 aspect-square text-11 text-tertiary hover:bg-custom-background-80 rounded-sm opacity-100 !outline-none z-[5] transition-[background-color,_opacity] duration-200 ease-linear";
