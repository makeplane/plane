...
export const BlockMathBlock = memo(function BlockMathBlock(props: TBlockMathBlockProps) {
  ...
  const validation = !isDisplayEmpty
    ? validateLaTeX(displayLatex, { displayMode: true })
  ...
  return (
    <>
      <div ref={blockRef} onClick={handleClick}>
        {isDisplayEmpty ? (
          <BlockMathEmptyState selected={isOpen || selected} editor={editor} isEditable={editor.isEditable} />
        ) : !validation.isValid ? (
          <BlockMathErrorState errorMessage={validation.errorMessage} isEditable={editor.isEditable} />
        ) : (
          <BlockMathView latex={displayLatex} isEditable={editor.isEditable} />
        )}
      </div>
      <FloatingMathModal
        isOpen={isOpen}
