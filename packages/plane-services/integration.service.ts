...
class IntegrationService extends APIService {
  ...
  async getWorkspaceIntegrationsList(
  ...
  async getImporterServicesList(
    workspaceSlug: string
  ): Promise<IImporterService[]> {
  ...
  async deleteImporterService(
    ...
    return this.delete(
      `/api/workspaces/${workspaceSlug}/importers/${service}/${importerId}/`
    )
      .then((response) => {
        const eventName =
          service === "github"
        ...
        if (trackEvent)
          trackEventServices.trackImporterEvent(
            response?.data,
            eventName,
            user
          );
