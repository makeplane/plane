...
export class FileService extends APIService {
  ...
  private async updateWorkspaceAssetStatus(workspaceSlug: string, assetId: string): Promise<void> {
  ...
  async upload(workspaceSlug: string, data: TFileEntityInfo, file: File): Promise<TFileSignedURLResponse> {
  ...
  async destroy(workspaceSlug: string, assetId: string): Promise<void> {
  ...
  private async updateProjectAssetStatus(workspaceSlug: string, projectId: string, assetId: string): Promise<void> {
  ...
  async uploadProjectAsset(
    ...
    return this.post(`/api/assets/v2/workspaces/${workspaceSlug}/projects/${projectId}/`, {
      ...
      .then(async (response) => {
        ...
        const fileUploadPayload = generateFileUploadPayload(signedURLResponse, file);
        await this.fileUploadService.uploadFile(signedURLResponse.upload_data.url, fileUploadPayload);
        await this.updateProjectAssetStatus(workspaceSlug, projectId, signedURLResponse.asset_id);
        return signedURLResponse;
      })
      .catch((error) => {
        throw error?.response?.data;
