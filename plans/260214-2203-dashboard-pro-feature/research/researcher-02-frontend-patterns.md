...
export class DashboardService extends APIService {
  ...
  async getDashboardData<T>(workspaceSlug: string, params?: Record<string, any>): Promise<T> {
  ...
  async getDashboardStats<T>(workspaceSlug: string, viewType: string): Promise<T> {
...
function DashboardPage({ params }: Route.ComponentProps) {
  ...
  return (
    <>
      ...
      <div className="flex h-full overflow-hidden">{/* Page content */}</div>
    </>
  );
}
