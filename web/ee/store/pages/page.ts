...
export class WorkspacePageDetails implements IWorkspacePageDetails {
  ...
  get isContentEditable() {
    ...
    const currentUserRole = this.store.user.permission.workspaceInfoBySlug(workspaceSlug?.toString() || "")?.role;
  ...
  update = async (pageData: Partial<TPage>) => {
    ...
    try {
      ...
      await this.pageService.update(workspaceSlug, this.id, currentPage);
    } catch (error) {
      runInAction(() => {
        Object.keys(pageData).forEach((key) => {
          const currentPageKey = key as keyof TPage;
  ...
  lock = async () => {
    ...
    await this.pageService.lock(workspaceSlug, this.id).catch((error) => {
      runInAction(() => {
  ...
  unlock = async () => {
    ...
    await this.pageService.unlock(workspaceSlug, this.id).catch((error) => {
      runInAction(() => {
        this.is_locked = pageIsLocked;
      });
      throw error;
  ...
  addToFavorites = async () => {
    ...
    await this.pageService.addToFavorites(workspaceSlug, this.id).catch((error) => {
      runInAction(() => {
  ...
  removePageFromFavorites = async () => {
    ...
    await this.pageService.removeFromFavorites(workspaceSlug, this.id).catch((error) => {
      runInAction(() => {
        this.is_favorite = pageIsFavorite;
      });
      throw error;
