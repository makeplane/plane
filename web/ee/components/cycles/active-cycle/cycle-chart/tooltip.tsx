  ...
  if (active && payload && payload.length) {
    ...
    return (
      <Card className="flex flex-col" spacing={ECardSpacing.SM}>
        <p className="text-xs text-custom-text-350 border-b pb-2">{`${day} ${monthName}'${parseInt(year) % 100}`}</p>
        <div className="flex flex-col space-y-2">
          <span className="flex text-xs text-custom-text-350 gap-1">
            <PlannedState className="my-auto" width="14" height="14" />
            <span className="font-semibold">{Math.round(payload.ideal) || 0}</span>
            <span> planned</span>
          </span>
          <span className="flex text-xs text-custom-text-350 gap-1 items-center">
            <InProgressState className="my-auto items-center" width="14" height="14" />
            <span className="font-semibold">{payload.started || 0}</span>
            <span> in-progress</span>
          </span>
          <span className="flex text-xs text-custom-text-350 gap-1 items-center ml-0.5">
            {plotType === "burndown" ? (
