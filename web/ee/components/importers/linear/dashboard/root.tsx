...
import { IconFieldRender, SyncJobStatus } from "@/plane-web/components/importers/linear";
// plane web hooks
...
export const Dashboard: FC = observer(() => {
  ...
  // fetching jobs
  useSWR(
    currentAuth?.isAuthenticated ? `IMPORTER_JOBS_LINEAR` : null,
    currentAuth?.isAuthenticated ? async () => await fetchJobs() : null,
    { errorRetryCount: 0 }
  );

  // derived values
  const isReRunDisabled = (job: TJobWithConfig<LinearConfig>) => {
  ...
  const handleDeactivateAuth = async () => {
    try {
      ...
      console.error("Error while deactivating Linear auth", error);
      setDeactivateLoader(false);
    }
  };
                  ...
                  jobIds.map((jobId, index) => {
                    ...
                    return (
                      <tr key={job.id} className="text-sm text-custom-text-200">
                        ...
                        <td className="p-3 whitespace-nowrap">
                          <IconFieldRender title={job?.config?.meta?.workspaceDetail?.name} />
                        </td>
                        <td className="p-3 whitespace-nowrap">
                          <IconFieldRender title={job?.config?.meta?.teamDetail?.name || job?.config?.meta?.teamName} />
                        </td>
