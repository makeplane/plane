...
export type InitiativeTitleInputProps = {
  disabled?: boolean;
...
export const InitiativeTitleInput: FC<InitiativeTitleInputProps> = observer((props) => {
  const { disabled, value, workspaceSlug, isSubmitting, setIsSubmitting, initiativeId, className, containerClassName } =
    props;
  // states
  ...
  useEffect(() => {
    ...
    if (debouncedValue && debouncedValue !== value) {
      if (debouncedValue.trim().length > 0) {
        updateInitiative(workspaceSlug, initiativeId, { name: debouncedValue }).finally(() => {
