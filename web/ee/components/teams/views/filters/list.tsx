...
type TeamViewListHeaderProps = {
  teamId: string;
};

export const TeamViewListHeader = observer((props: TeamViewListHeaderProps) => {
  const { teamId } = props;
  // states
  ...
  // store hooks
  const { getTeamViewsFilters, updateFilters: updateTeamViewFilters } = useTeamViews();
  const {
    workspace: { workspaceMemberIds },
  } = useMember();
  // derived values
  const filters = getTeamViewsFilters(teamId);

  // handlers
  const updateFilters = <T extends keyof TViewFilters>(filterKey: T, filterValue: TViewFilters[T]) => {
    updateTeamViewFilters(teamId, filterKey, filterValue);
  };
