...
// types
import { TPublishViewSettings, TTeamView } from "@plane/types";
// ui
...
export const PublishTeamViewModal: React.FC<Props> = observer((props) => {
  const { isOpen, view, teamId, onClose } = props;
  // states
  const [isUnPublishing, setIsUnPublishing] = useState(false);
  // router
  const { workspaceSlug } = useParams();
  // store hooks
  const { publishView, fetchPublishDetails, updatePublishedView, unPublishView } = useTeamViews();
  ...
  const { data: publishedViewSettings, isLoading } = useSWR(
    ...
    view?.anchor && isViewsPublishEnabled ? () => fetchPublishDetails(workspaceSlug.toString(), teamId, view.id) : null
  );
  // form info
  const {
    control,
  ...
  const handlePublishView = async (payload: TPublishViewSettings) => {
    ...
    await publishView(workspaceSlug.toString(), teamId, view.id, payload);
  };

  const handleUpdatePublishSettings = async (payload: Partial<TPublishViewSettings>) => {
    if (!workspaceSlug || !view) return;

    await updatePublishedView(workspaceSlug.toString(), teamId, view.id, payload).then((res) => {
      setToast({
  ...
  const handleUnPublishView = async () => {
    ...
    await unPublishView(workspaceSlug.toString(), teamId, view.id)
      .catch(() =>
        setToast({
