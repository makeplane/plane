...
export const DeleteTeamViewModal: React.FC<Props> = observer((props) => {
  const { data, isOpen, onClose, teamId } = props;
  // states
  const [isDeleteLoading, setIsDeleteLoading] = useState(false);
  // router
  const { workspaceSlug } = useParams();
  // store hooks
  const { deleteView } = useTeamViews();
  ...
  const handleDeleteView = async () => {
    ...
    await deleteView(workspaceSlug.toString(), teamId, data.id)
      .then(() => {
        handleClose();
