...
import { TeamCommentBlock } from "./comment-block";
import { TeamCommentReactions } from "./comment-reaction";

type TTeamCommentCard = {
...
export const TeamCommentCard: FC<TTeamCommentCard> = observer((props) => {
  const { workspaceSlug, comment, teamId, activityOperations, showAccessSpecifier = false, disabled = false } = props;
  // refs
  ...
  // helpers
  const onEnter = (formData: Partial<TTeamComment>) => {
    ...
    >
      <>
        ...
        <div className={`relative ${isEditing ? "hidden" : ""}`}>
          ...
          />
          <TeamCommentReactions workspaceSlug={workspaceSlug} teamId={teamId} comment={comment} disabled={disabled} />
        </div>
      </>
    </TeamCommentBlock>
  );
