...
// plane types
import { TTeamView, TViewFilterProps, TViewFiltersSortBy, TViewFiltersSortKey } from "@plane/types";
// helpers
...
export const orderViews = (
  ...
  if (sortByKey === "name") {
    iterableFunction = (view: TTeamView) => view.name?.toLowerCase();
  }
  if (sortByKey === "created_at") {
    iterableFunction = (view: TTeamView) => view.created_at;
  }
  if (sortByKey === "updated_at") {
    iterableFunction = (view: TTeamView) => view.updated_at;
  }
 ...
 * @returns
 */
export const shouldFilterView = (view: TTeamView, filters: TViewFilterProps | undefined): boolean => {
 ...
 * @returns
 */
export const getValidatedViewFilters = (data: Partial<TTeamView>) => {
