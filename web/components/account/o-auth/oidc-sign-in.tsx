...
export const OidcSignInButton: FC<OidcSignInButtonProps> = (props) => {
  const { handleSignInRedirection, autoRedirect, clientId, authUrl, type } = props;
  // router
  const {
    push: routerPush,
    query: { code },
        ...
        >
          {(autoRedirect && !initialSignInError) || code ? (
            ...
            <>
              ...
              <span className="text-onboarding-text-200">{type === "sign_in" ? "Sign-in" : "Sign-up"} with OIDC</span>
            </>
          )}
        </button>
      </Link>
