...
export const CalendarWeekHeader: React.FC<Props> = observer((props) => {
  ...
  return (
    <div
      className={`relative grid md:divide-x-[0.5px] divide-custom-border-200 text-sm font-medium ${
    ...
    >
      ...
      {Object.values(DAYS_LIST).map((day) => {
        ...
        return (
          <div key={day.shortTitle} className="flex h-11 items-center justify-center md:justify-start bg-custom-background-90 px-4">
            {day.shortTitle}
          </div>
        );
      })}
