...
export const generateCalendarData = (currentStructure: ICalendarPayload | null, startDate: Date): ICalendarPayload => {
  ...
  const firstDayOfWeek = new Date(year, month, 1).getDay(); // Sunday is 0, Monday is 1, ..., Saturday is 6
  ...
  for (let week = 0; week < numWeeks; week++) {
    ...
    const weekNumber = getWeekNumberOfDate(new Date(year, month, week * 7 - firstDayOfWeek + 1));

    for (let i = 0; i < 7; i++) {
      const dayNumber = week * 7 + i - firstDayOfWeek;
    ...
    calendarData[year][month][weekNumber] = currentWeekObject;
  }
