...
export const InsightTable = <T extends Exclude<TAnalyticsTabsV2Base, "overview">>(
    ...
    const exportCSV = (rows: Row<AnalyticsTableDataMap[T]>[]) => {
        const rowData: any = rows.map((row) => {
            ...
            return Object.fromEntries(Object.entries(exportableData).map(([key, value]) => {
    ...
    return (
        <div className="">
            {data ? <DataTable
                ...
                actions={(table: Table<AnalyticsTableDataMap[T]>) => <Button variant="accent-primary" prependIcon={<Download className="h-3.5 w-3.5" />} onClick={() => exportCSV(table.getFilteredRowModel().rows)}>
