...
type SettingsSidebarProps = {
  ...
  shouldRender: (data: { key: string; access: EUserWorkspaceRoles[] }) => boolean;
};

const SettingsSidebar = (props: SettingsSidebarProps) => {
  const { categories, groupedSettings, workspaceSlug, children, customHeader, isActive, shouldRender } = props;
            ...
            <div key={category} className="py-3">
              ...
              <div className="flex flex-col gap-0.5">
                {groupedSettings[category].map(
                  (setting) =>
                    shouldRender(setting) && (
                      <div key={setting.key}>
                        <Link
                          href={`/${workspaceSlug}/${setting.href}`}
                          className={cn(
                            "flex items-center px-2 py-1.5 rounded text-custom-text-200",
