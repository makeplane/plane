...
export const FilterCompletedAt: React.FC<Props> = observer((props) => {
  ...
  const isCustomDateSelected = () => {
    const isCustomDateApplied = appliedFilters?.filter((f) => f.includes("-")) || [];
    return isCustomDateApplied.length > 0 ? true : false;
  };
  ...
  return (
    <>
      ...
      <FilterHeader
        title={`Completed date${appliedFiltersCount > 0 ? ` (${appliedFiltersCount})` : ""}`}
