...
def postprocess_assign_tags(result, generator, request, public):
    ...
    tag_mappings = [
        {
            'patterns': ['/projects/{project_id}/cycles/', '/cycles/{cycle_id}/', '/archived-cycles/', '/cycle-issues/'],
            'tag': 'Cycles'
        },
        {
            'patterns': ['/projects/{project_id}/modules/', '/modules/{module_id}/', '/archived-modules/', '/module-issues/'],
            'tag': 'Modules'
        },
        {
            'patterns': ['/projects/{project_id}/issues/', '/issues/{issue_id}/', '/intake-issues/', '/issue-attachments/'],
            'tag': 'Issues'
        },
        {
        ...
        {
            'patterns': ['/projects/{project_id}/labels/', '/labels/{'],
            'tag': 'Labels'
        },
        {
            'patterns': ['/members/', '/members/{'],
            'tag': 'Members'
        },
        {
            'patterns': ['/assets/', '/user-assets/', '/generic-asset'],
            'tag': 'Assets'
        },
        {
        ...
        {
            'patterns': ['/transfer-issues/', '/transfer/'],
            'tag': 'Issues'  # Transfer endpoints are issue-related
        },
        {
            'patterns': ['/projects/', '/projects/{', '/archive/'],
            'tag': 'Projects'
        }
    ]
        ...
        for method, operation in path_info.items():
            if method.upper() in ['GET', 'POST', 'PATCH', 'DELETE']:
                ...
                if 'summary' not in operation:
                    operation['summary'] = generate_operation_summary(method.upper(), path, assigned_tag)
